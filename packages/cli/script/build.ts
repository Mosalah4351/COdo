...
for (const item of targets) {
  ...
  const result = await Bun.build({
    ...
    define: {
      ...
      COdo_CLI_NAME: `'${binary}'`,
      COdo_MODELS_DEV: modelsData,
      COdo_CHANNEL: `'${Script.channel}'`,
      COdo_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "undefined",
    ...
    JSON.stringify(
      {
        ...
        license: "MIT",
        repository: { type: "git", url: "git+https://github.com/anomalyco/COdo.git" },
        os: [item.os],
        cpu: [item.arch],
      },
      null,
