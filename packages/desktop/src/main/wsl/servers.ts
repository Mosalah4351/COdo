...
import { expectCOdoVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolveCOdo?: typeof resolveWslCOdo
...
export function createWslServersController(
  ...
  const setCOdoCheck = (distro: string, check: WslCOdoCheck) => {
    setState({
      COdoChecks: {
        ...state.COdoChecks,
        [distro]: check,
  ...
  const checkCOdo = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveCOdo ?? resolveWslCOdo)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return COdoCheck(distro, resolved, version, appVersion)
  }

  const refreshCOdoCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setCOdoCheck(distro, await checkCOdo(distro, opts))
  }
  ...
  const refreshCOdoCheckBackground = (id: string, distro: string) => {
    void checkCOdo(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setCOdoCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl COdo check failed", { id, distro, message })
      })
  }

  const refreshCOdoChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkCOdo(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setCOdoCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl COdo check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshCOdoCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async probeCOdo(name: string) {
      await runJob({ kind: "probe-COdo", distro: name, startedAt: Date.now() }, async (abort) => {
        await refreshCOdoCheck(name, { signal: abort.signal })
      })
    },

    async installCOdo(name: string) {
      await runJob({ kind: "install-COdo", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslCOdo(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "COdo installation failed")
        }
        await refreshCOdoCheck(name, { signal: abort.signal })
        expectCOdoVersion(state.COdoChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.COdoChecks, distro) : {}),
      })
    },
...
function COdoCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "COdo is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "COdo is installed but could not run",
    }
  }
  return {
    distro,
