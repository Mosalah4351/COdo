...
import { type WslCommandLine, resolveWslCOdo, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const COdo = await resolveWslCOdo(distro)
  if (!COdo) throw new Error(`COdo is not installed in ${distro}`)
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export COdo_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export COdo_CLIENT=desktop",
    `export COdo_SERVER_USERNAME=${shellEscape(username)}`,
    `export COdo_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(COdo)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
