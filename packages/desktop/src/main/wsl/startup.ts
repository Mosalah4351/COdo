...
export function expectCOdoVersion(installed: string | null, expected: string, distro = "Debian") {
  if (installed === expected) return
  throw new Error(
    `COdo update finished but ${distro} still reports ${installed ?? "no version"}; expected ${expected}`,
  )
}
