...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.COdo.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `COdo.settings.dat` is special: it maps to the `COdo.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"COdo.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // COdo.settings.dat → the electron settings store ("COdo.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "COdo.global.dat").
  const storeName = filename === "COdo.settings.dat" ? "COdo.settings" : filename
