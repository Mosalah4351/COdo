  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://COdo.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://COdo.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://COdo.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://COdo.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://COdo.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://COdo.example.com"),
      ),
    ).toEqual({
