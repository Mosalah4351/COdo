import { Button } from "@codo-ai/ui/button"
import { useDialog } from "@codo-ai/ui/context/dialog"
import { Spinner } from "@codo-ai/ui/spinner"
import { showToast } from "@codo-ai/ui/toast"
import { createEffect, createMemo, For, Match, onCleanup, Show, Switch } from "solid-js"
import { createStore } from "solid-js/store"
import { useLanguage } from "@/context/language"
import { usePlatform } from "@/context/platform"
import { useWslServers } from "./context"
import { enterWslCOdoStep } from "./settings-model"

type WslServerStep = "wsl" | "distro" | "COdo"

const STEPS: WslServerStep[] = ["wsl", "distro", "COdo"]
...
export function DialogAddWslServer(props: DialogWslServerProps = {}) {
  ...
  const COdoCheck = createMemo(() => {
    const distro = selectedDistro()
    if (!distro) return null
    return current()?.COdoChecks[distro] ?? null
  })
  ...
  const COdoReady = createMemo(() => {
    const check = COdoCheck()
  ...
  const installingCOdo = createMemo(() => {
    const job = current()?.job
    return job?.kind === "install-COdo" && job.distro === selectedDistro()
  })
  const allReady = createMemo(() => wslReady() && distroReady() && COdoReady())
  const addDisabled = createMemo(() => {
    const job = current()?.job
    if (!job) return store.adding
    return store.adding || job.kind !== "probe-COdo"
  })
  ...
  const autoProbe = createMemo(() => {
    ...
    if (!state.COdoChecks[distro]) {
      return { key: `probe-COdo:${distro}`, run: () => api.probeCOdo(distro) }
    }
    return null
  })
  ...
  const COdoMessage = createMemo(() => {
    const state = current()
    if (!state) return language.t("wsl.onboarding.checkingCOdo")
    const distro = selectedDistro()
    if (state.job?.kind === "install-COdo") {
      return distro
        ? language.t("wsl.onboarding.updatingCOdoIn", { distro })
        : language.t("wsl.onboarding.updatingCOdo")
    }
    if (state.job?.kind === "probe-COdo") {
      return distro
        ? language.t("wsl.onboarding.checkingCOdoIn", { distro })
        : language.t("wsl.onboarding.checkingCOdo")
    }
    if (COdoCheck()?.error) return COdoCheck()!.error
    if (COdoCheck()?.matchesDesktop === false) {
      return distro
        ? language.t("wsl.onboarding.updateCOdoIn", { distro })
        : language.t("wsl.onboarding.updateCOdo")
    }
    if (COdoReady()) {
      return distro
        ? language.t("wsl.onboarding.COdoReadyIn", { distro })
        : language.t("wsl.onboarding.COdoReady")
    }
    return distro
      ? language.t("wsl.onboarding.installCOdoIn", { distro })
  ...
  const openCOdoStep = () => {
    const distro = selectedDistro()
    if (!distro) return
    void run(() => enterWslCOdoStep(distro, api.probeCOdo, (step) => setStore("step", step)))
  }
        ...
        title:
          step === "wsl"
            ...
            : step === "distro"
              ...
              : language.t("wsl.onboarding.step.COdo"),
        state:
          active === step
            ? "current"
            : step === "wsl"
              ...
              : step === "distro"
                ? distroReady()
                  ...
                  : index > activeIndex
                    ? "locked"
                    : "warning"
                : COdoCheck()?.matchesDesktop === false
                  ? "warning"
                  : COdoReady()
                    ? "done"
                    : index > activeIndex
      ...
      >
        ...
        >
          ...
          <Switch>
            ...
            <Match when={activeStep() === "COdo"}>
              <div class="rounded-md bg-surface-base p-4 flex flex-col gap-3">
                <div class="flex items-center justify-between gap-3">
                  <div class="text-14-medium text-text-strong">{language.t("wsl.onboarding.step.COdo")}</div>
                  <div class="flex items-center gap-2">
                    <Show when={selectedDistro()}>
                      <Button
                        variant="ghost"
                        size="large"
                        disabled={busy()}
                        onClick={() => runSelectedDistro((distro) => api.probeCOdo(distro))}
                      >
                        {language.t("wsl.onboarding.refresh")}
                      </Button>
                    </Show>
                    <Show when={!COdoReady() || COdoCheck()?.matchesDesktop === false}>
                      <Button
                        variant="secondary"
                        size="large"
                        disabled={busy()}
                        onClick={() => runSelectedDistro((distro) => api.installCOdo(distro))}
                      >
                        <Show when={installingCOdo()}>
                          <Spinner class="size-4 shrink-0" />
                        </Show>
                        {COdoCheck()?.resolvedPath
                          ? language.t("wsl.onboarding.updateCOdo")
                          : language.t("wsl.onboarding.installCOdo")}
                      </Button>
                    </Show>
                  </div>
                </div>
                <div class="text-12-regular text-text-weak whitespace-pre-wrap break-words">{COdoMessage()}</div>
                <Show when={COdoCheck()?.matchesDesktop === false ? COdoCheck() : null}>
                  {(check) => (
                    <div class="rounded-md border border-border-weak-base px-3 py-3 flex flex-col gap-1">
                      <div class="text-12-regular text-text-weak">
          ...
          <Show when={activeStep() === "COdo" && allReady() && selectedDistro()}>
