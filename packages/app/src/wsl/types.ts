...
export type WslJob =
  ...
  | { kind: "probe-COdo"; distro: string; startedAt: number }
  | { kind: "install-COdo"; distro: string; startedAt: number }
...
export type WslServersPlatform = {
  ...
  probeCOdo(name: string): Promise<void>
  installCOdo(name: string): Promise<void>
