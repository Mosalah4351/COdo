import { createEffect, createMemo, createRoot, For, onCleanup, onMount } from "solid-js"
...
function SessionTabSlot(props: {
  ...
  const directory = createMemo(() => atob(props.tab.dirBase64))
  const dirSync = createMemo(() => {
    const ctx = props.serverCtx()
    if (!ctx) return undefined
    return ctx.sync.createDirSyncContext(directory())
  })
  const session = createMemo(() => dirSync()?.session.get(props.tab.sessionId))
  let prefetched = false

  createEffect(() => {
    const sync = dirSync()
    if (!sync || prefetched) return
    prefetched = true
    createRoot((dispose) => {
      try {
        void sync.session
          .sync(props.tab.sessionId)
          .catch(() => {})
          .finally(dispose)
      } catch {
        dispose()
