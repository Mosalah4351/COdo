...
import { Accordion } from "@codo-ai/ui/accordion"
import { Button } from "@codo-ai/ui/button"
import { Card } from "@codo-ai/ui/card"
import {
  ContextToolGroup,
  Message,
  MessageDivider,
  ...
  partDefaultOpen,
  type UserActions,
} from "@codo-ai/session-ui/message-part"
import { DiffChanges } from "@codo-ai/ui/diff-changes"
import { FileIcon } from "@codo-ai/ui/file-icon"
import { Icon } from "@codo-ai/ui/icon"
import { IconButton } from "@codo-ai/ui/icon-button"
import { DropdownMenu } from "@codo-ai/ui/dropdown-menu"
import { Dialog } from "@codo-ai/ui/dialog"
import { InlineInput } from "@codo-ai/ui/inline-input"
import { SessionRetry } from "@codo-ai/session-ui/session-retry"
import { ScrollView } from "@codo-ai/ui/scroll-view"
import { StickyAccordionHeader } from "@codo-ai/ui/sticky-accordion-header"
import { TextField } from "@codo-ai/ui/text-field"
import { TextReveal } from "@codo-ai/ui/text-reveal"
import { TextShimmer } from "@codo-ai/ui/text-shimmer"
import type {
  AssistantMessage,
...
import { getDirectory, getFilename } from "@codo-ai/core/util/path"
import { Popover as KobaltePopover } from "@kobalte/core/popover"
import { normalize } from "@codo-ai/session-ui/session-diff"
import { useFileComponent } from "@codo-ai/ui/context/file"
...
export function MessageTimeline(props: {
  ...
  const archiveSession = async (sessionID: string) => {
    ...
    await sdk.client.session.update({ sessionID, time: { archived: Date.now() } })
      .then(() => {
        ...
        notifySessionTabsRemoved({ server: params.serverKey ? requireServerKey(params.serverKey) : server.key, directory: sdk.directory, sessionIDs: [sessionID] })
      })
      .catch((err) => {
        showToast({
          title: language.t("common.requestFailed"),
  ...
  const deleteSession = async (sessionID: string) => {
    ...
    notifySessionTabsRemoved({ server: params.serverKey ? requireServerKey(params.serverKey) : server.key, directory: sdk.directory, sessionIDs: [...removed] })
    return true
  }
