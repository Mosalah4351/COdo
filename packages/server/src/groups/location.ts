...
function ref(request: HttpServerRequest.HttpServerRequest): Location.Ref {
  ...
  const workspaceID = query.get("location[workspace]") || request.headers["x-COdo-workspace"]
  const directory =
    query.get("location[directory]") ||
    (request.headers["x-COdo-directory"] ? decode(request.headers["x-COdo-directory"]) : process.cwd())
