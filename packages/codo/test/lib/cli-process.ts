// Subprocess test harness for the COdo CLI. Spawns the real binary against
...
// builders (`COdo.serve(opts)`, `COdo.acp(opts)`, `COdo.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of COdoCli.
...
const COdoRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(COdoRoot, "src/index.ts")
...
// Typed equivalent of constructing argv for `COdo run`. New flags should
...
// `COdo serve` is a long-lived process — it never exits on its own.
...
// `COdo acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type COdoCli = {
  ...
  // Spawn `COdo serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `COdo acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("COdo.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const serve = Effect.fn("COdo.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "COdo server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `COdo serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("COdo.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const COdo: COdoCli = { run, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, COdo })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `COdo.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "COdo") {
...
// every call site. Use this for any test that needs the COdo CLI fixture.
//
...
// resources (e.g. `COdo.serve`) without an extra `Effect.scoped` wrapper —
