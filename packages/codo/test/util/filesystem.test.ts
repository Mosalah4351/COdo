...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "codo.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "codo.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "codo.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "codo.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["codo.json", "codo.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "codo.json"),
        path.join(tmp.path, "codo.jsonc"),
        path.join(project, "codo.json"),
        path.join(project, "codo.jsonc"),
      ])
    })
  })
