...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.CODO_WEBSEARCH_PROVIDER

    try {
      process.env.CODO_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.CODO_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.CODO_WEBSEARCH_PROVIDER
      else process.env.CODO_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is only enabled for codo or explicit websearch provider flags", () => {
    expect(webSearchEnabled(ProviderV2.ID.codo, { exa: false, parallel: false })).toBe(true)
