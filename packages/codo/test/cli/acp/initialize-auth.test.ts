  ...
  cliIt.live(
    ...
    ({ COdo }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.agentInfo?.name).toBe("codo")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ COdo }) =>
      Effect.gen(function* () {
        ...
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "codo-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.CODO_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
