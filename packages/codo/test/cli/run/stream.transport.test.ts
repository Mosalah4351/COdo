...
import { COdoClient, type GlobalEvent } from "@codo-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<COdoClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<COdoClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<COdoClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<COdoClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<COdoClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://codo.test"),
...
function sdk(
  input: {
    ...
    subscribe?: COdoClient["event"]["subscribe"]
    globalEvent?: COdoClient["global"]["event"]
    promptAsync?: COdoClient["session"]["promptAsync"]
    status?: COdoClient["session"]["status"]
    messages?: COdoClient["session"]["messages"]
    children?: COdoClient["session"]["children"]
    permissions?: COdoClient["permission"]["list"]
    questions?: COdoClient["question"]["list"]
  } = {},
) {
  const client = new COdoClient()

  const subscribe: COdoClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: COdoClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: COdoClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: COdoClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: COdoClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: COdoClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: COdoClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: COdoClient["question"]["list"] = input.questions ?? (() => ok([]))
