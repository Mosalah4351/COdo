...
// terminal sizes. The default codo tui command is excluded —
// `codo --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `codo --help` prints today;
...
// top-level help on `--help` and exits 1; not a real codo command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("codo CLI help-text snapshots", () => {
    ...
    ({ COdo }) =>
      Effect.gen(function* () {
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`codo ${argv.join(" ")} --help`)
        }
