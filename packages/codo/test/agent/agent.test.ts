...
it.instance("plan agent denies edits except .codo/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".codo/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".codo", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.CODO_TEST_HOME
      process.env.CODO_TEST_HOME = test.directory
