...
type Sdk = ReturnType<typeof createCOdoClient>
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.CODO_SERVER_PASSWORD = input?.password
      Flag.CODO_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".codo", "skills", "project-rest-skill", "SKILL.md"),
      `---
  ...
  httpapi(
    ...
    withProject("raw", { setup: writeStandardFiles }, ({ directory }) =>
      Effect.gen(function* () {
        ...
        expect(request!.headers.has("x-codo-directory")).toBe(false)
        expect(request!.headers.has("x-codo-workspace")).toBe(false)
      }),
    ),
  )
