...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-codo-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-codo-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://codo-ai.vercel.app/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-codo-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-codo-workspace")
