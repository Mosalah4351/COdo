...
import type { Message, COdoClient, SessionMessageResponse } from "@codo-ai/sdk/v2"
...
export const AuthMethodID = "COdo-login"
...
export class Service extends Context.Service<Service, Interface>()("@codo/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `COdo auth login` in the terminal",
      name: "Login with COdo",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: COdoClient) {
...
function makeUsageService(sdk: COdoClient) {
...
async function loadDirectorySnapshot(sdk: COdoClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, COdo provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const COdoProvider = providers[ProviderV2.ID.make("COdo")]
  const COdoModel = COdoProvider ? Provider.sort(Object.values(COdoProvider.models))[0] : undefined
  if (COdoProvider && COdoModel) return { providerID: COdoProvider.id, modelID: COdoModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "COdo service failure", service })
}
