...
export const DEPRECATED_PLUGIN_PACKAGES = ["COdo-openai-codex-auth", "COdo-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, COdoVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(COdoVersion) || semver.major(COdoVersion) === 0) return
