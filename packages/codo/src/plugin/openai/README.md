...
Enabled by default on `local`, `dev`, and `beta`. On `latest` and `prod`, set `CODO_EXPERIMENTAL_WEBSOCKETS=true`. (Prior name was `OPENCODE_EXPERIMENTAL_WEBSOCKETS`; the COdo-prefixed name is canonical going forward.)
