...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall COdo and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g COdo-ai",
      pnpm: "pnpm uninstall -g COdo-ai",
      bun: "bun remove -g COdo-ai",
      yarn: "yarn global remove COdo-ai",
      brew: "brew uninstall COdo",
      choco: "choco uninstall COdo",
      scoop: "scoop uninstall COdo",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "COdo", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using COdo!")
}
