...
const TUI_SCHEMA_URL = "https://COdo.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from COdo.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const COdo = await COdoFiles(input)
  for (const file of COdo) {
...
async function COdoFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "COdo"),
    ...(await Filesystem.findUp(["COdo.json", "COdo.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
