...
import { InstallationLocal, InstallationVersion } from "@codo-ai/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@codo-ai/core/v1/config/console-state"
...
import { RemoteAuthError } from "@codo-ai/core/v1/config/error"
import { ConfigPermissionV1 } from "@codo-ai/core/v1/config/permission"
...
export class Service extends Context.Service<Service, Interface>()("@codo/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["COdo.jsonc", "COdo.json", "config.json"].map((file) =>
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.CODO_CONFIG && !Flag.CODO_CONFIG_DIR && !Flag.CODO_CONFIG_CONTENT) {
        ...
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://COdo.ai/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "COdo.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "COdo.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://COdo.ai/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "CODO_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/COdo`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://COdo.ai/config.json"
        ...
        if (process.env.CODO_CONFIG_CONTENT) {
          const source = "CODO_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.CODO_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["CODO_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("CODO_CONSOLE_TOKEN", tokenOpt.value)
            }
