...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for COdo serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(COdoHttpApi)
...
export const PublicApi = COdoHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "COdo",
    version: "1.0.0",
    description: "COdo api",
