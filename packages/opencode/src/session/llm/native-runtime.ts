...
import type { LLMClientShape } from "@codo-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("COdo"))
    return { type: "unsupported", reason: "provider is not openai, COdo, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @codo-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains COdo-owned. The native runtime only adapts
      // the @codo-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
