...
import type { COdoClient, PermissionRequest, QuestionRequest, ToolPart } from "@codo-ai/sdk/v2"
import type { TuiConfig } from "@codo-ai/tui/config"
...
type PromptModel = Parameters<COdoClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<COdoClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<COdoClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<COdoClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<COdoClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<COdoClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<COdoClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<COdoClient["question"]["reply"]>[0]
