import type { PermissionV1 } from "@codo-ai/core/v1/permission"
// CLI entry point for `COdo run`.
//
// Handles three modes:
...
//      COdo server and runs interactive mode against it.
//
...
import { createCOdoClient, type COdoClient, type ToolPart } from "@codo-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<COdoClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run COdo with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running COdo server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: COdoClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: COdoClient, sessionID: string) {
      ...
      async function current(sdk: COdoClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: COdoClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: COdoClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: COdoClient) {
        ...
        async function loop(client: COdoClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
