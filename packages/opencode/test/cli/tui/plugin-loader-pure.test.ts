...
`,
  ...
  const pure = process.env.CODO_PURE
  const meta = process.env.CODO_PLUGIN_META_FILE
  process.env.CODO_PURE = "1"
  process.env.CODO_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.CODO_PURE
    } else {
      process.env.CODO_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.CODO_PLUGIN_META_FILE
    } else {
      process.env.CODO_PLUGIN_META_FILE = meta
    }
  }
})
