---
title: 規則
description: 為 COdo 設定自訂指令。
---

您可以透過建立 `AGENTS.md` 檔案來為 COdo 提供自訂指令。這類似於 Cursor 的規則功能。該檔案包含的指令會被納入 LLM 的上下文中，以便針對您的特定專案自訂其行為。

---

## 初始化

要建立新的 `AGENTS.md` 檔案，您可以在 COdo 中執行 `/init` 指令。

:::tip
您應該將專案的 `AGENTS.md` 檔案提交到 Git。
:::

該指令會掃描您的專案及其所有內容，了解專案的用途，並據此產生一個 `AGENTS.md` 檔案。這有助於 COdo 更好地導覽您的專案。
...
COdo 還支援從多個位置讀取 `AGENTS.md` 檔案，不同的位置有不同的用途。
...
對於從 Claude Code 遷移過來的使用者，COdo 支援 Claude Code 的檔案慣例作為備援方案：
...
雖然 COdo 不會自動解析 `AGENTS.md` 中的檔案參照，但您可以透過以下兩種方式實現類似的功能：

### 使用 COdo.json

建議的方式是使用 `COdo.json` 中的 `instructions` 欄位：
...
您可以在 `AGENTS.md` 中提供明確的指令，教 COdo 讀取外部檔案。以下是一個實際範例：
...
- 確保 COdo 僅在特定任務需要時才載入檔案

:::tip
對於 monorepo 或具有共享標準的專案，使用 `COdo.json` 搭配 glob 模式（如 `packages/*/AGENTS.md`）比手動指定指令更易於維護。
:::
