---
title: 設定
description: 使用 COdo JSON 設定。
---

您可以使用 JSON 設定檔來設定 COdo。

---

## 格式

COdo 支援 **JSON** 和 **JSONC**（帶註解的 JSON）格式。
...
組織可以透過 `.well-known/COdo` 端點提供預設設定。當您使用支援該功能的供應商進行身分驗證時，會自動擷取此設定。
...
將全域 COdo 設定放在 `~/.config/opencode/opencode.json` 中。使用全域設定來設定使用者層級的偏好，例如主題、供應商或快捷鍵。
...
在專案根目錄中新增 `COdo.json`。專案設定在標準設定檔中具有最高優先級——它會覆寫全域設定和遠端設定。

:::tip
將專案特定設定放在專案的根目錄中。
:::

當 COdo 啟動時，它會在當前目錄中尋找設定檔，或向上遍歷到最近的 Git 目錄。
...
設定檔具有在 [**`codo-ai.vercel.app/config.json`**](https://codo-ai.vercel.app/config.json) 中定義的 Schema。
...
`COdo.json` 中的舊版 `theme`、`keybinds` 和 `tui` 鍵已被棄用，並將在可能的情況下自動遷移。
...
您可以透過 `server` 選項為 `COdo serve` 和 `COdo web` 指令設定伺服器設定。
...
- `mdns` - 啟用 mDNS 服務探索。這允許網路上的其他裝置發現您的 COdo 伺服器。
- `mdnsDomain` - mDNS 服務的自訂網域名稱。預設為 `COdo.local`。適用於在同一網路上執行多個實例的情境。
...
您可以透過 `provider`、`model` 和 `small_model` 選項在 COdo 設定中設定要使用的供應商和模型。
...
`small_model` 選項為標題生成等輕量級任務設定單獨的模型。預設情況下，如果您的供應商有更便宜的模型可用，COdo 會嘗試使用該模型，否則會回退到您的主模型。
...
預設代理必須是主代理（不能是子代理）。可以是內建代理（如 `"build"` 或 `"plan"`），也可以是您定義的[自訂代理](/docs/agents)。如果指定的代理不存在或是子代理，COdo 將回退到 `"build"` 並發出警告。

此設定適用於所有介面：TUI、CLI（`COdo run`）、桌面應用程式和 GitHub Action。
...
COdo 啟動時會自動下載新版本。您可以使用 `autoupdate` 選項停用此功能。
...
預設情況下，COdo **允許所有操作**，無需明確批准。您可以使用 `permission` 選項變更此行為。
...
[外掛程式](/docs/plugins)透過自訂工具、掛鉤和整合來擴展 COdo。
...
當您希望限制 COdo 僅使用特定供應商，而不是逐一停用其他供應商時，此選項非常有用。
