---
title: CLI
description: COdo CLI options and commands.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

The COdo CLI by default starts the [TUI](/docs/tui) when run without any arguments.

```bash
COdo
```

But it also accepts commands as documented on this page. This allows you to interact with COdo programmatically.
...
Start the COdo terminal user interface.
...
The COdo CLI also has the following commands.

---

### agent

Manage agents for COdo.
...
Attach a terminal to an already running COdo backend server started via `serve` or `web` commands.

```bash
COdo attach [url]
```

This allows using the TUI with a remote COdo backend. For example:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `OPENCODE_SERVER_USERNAME` or `COdo`) |
...
COdo is powered by the provider list at [Models.dev](https://models.dev), so you can use `opencode auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/opencode/auth.json`.

```bash
COdo auth login
```

When COdo starts up it loads the providers from the credentials file. And if there are any keys defined in your environments or a `.env` file in your project.
...
Use the `--refresh` flag to update the cached model list. This is useful when new models have been added to a provider and you want to see them in COdo.
...
Run COdo in non-interactive mode by passing a prompt directly.
...
You can also attach to a running `COdo serve` instance to avoid MCP server cold boot times on every run:
...
| <nobr><code>{"--attach"}</code></nobr>                       |       | Attach to a running COdo server (e.g., http://localhost:4096)          |
| <nobr><code>{"--password"}</code></nobr>                     | `-p`  | Basic auth password (defaults to `OPENCODE_SERVER_PASSWORD`)               |
| <nobr><code>{"--username"}</code></nobr>                     | `-u`  | Basic auth username (defaults to `OPENCODE_SERVER_USERNAME` or `COdo`) |
...
Start a headless COdo server for API access. Check out the [server docs](/docs/server) for the full HTTP interface.

```bash
COdo serve
```

This starts an HTTP server that provides API access to COdo functionality without the TUI interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `COdo`).
...
Delete an COdo session.

```bash
COdo session delete <sessionID>
```
...
Show token usage and cost statistics for your COdo sessions.
...
Import session data from a JSON file or COdo share URL.

```bash
COdo import <file>
```

You can import from a local file or an COdo share URL.
...
Start a headless COdo server with a web interface.

```bash
COdo web
```

This starts an HTTP server and opens a web browser to access COdo through a web interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `COdo`).
...
Fetch and checkout a GitHub PR branch, then run COdo.
...
Uninstall COdo and remove all related files.
...
Updates COdo to the latest version or a specific version.
...
The COdo CLI takes the following global flags.
...
COdo can be configured using environment variables.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Override basic auth username (default `COdo`) |
