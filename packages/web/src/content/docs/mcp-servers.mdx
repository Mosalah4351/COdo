...
You can add external tools to COdo using the _Model Context Protocol_, or MCP. COdo supports both local and remote servers.
...
You can define MCP servers in your [COdo Config](https://codo-ai.vercel.app/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/COdo` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
COdo automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, COdo will:
...
If the server requires authentication, COdo will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `COdo mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, COdo will store the tokens securely in `~/.local/share/opencode/mcp-auth.json`.
...
Your MCPs are available as tools in COdo, alongside built-in tools. So you can manage them through the COdo config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect COdo to your Sentry account.
