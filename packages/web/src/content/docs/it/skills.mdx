...
Le skill degli agenti permettono a COdo di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
COdo cerca in queste posizioni:
...
Per i percorsi locali al progetto, COdo risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
...
COdo elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `COdo.json`:
...
**Per agenti integrati** (in `COdo.json`):
