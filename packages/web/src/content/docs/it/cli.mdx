---
title: CLI
description: Opzioni e comandi della CLI di COdo.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di COdo, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
COdo
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con COdo in modo programmatico.
...
Avvia l'interfaccia testuale di COdo (interfaccia utente da terminale).
...
La CLI di COdo include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per COdo.
...
Collega un terminale a un backend server di COdo già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
COdo attach [url]
```

Questo consente di usare la TUI con un backend COdo remoto. Per esempio:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `COdo`) |
...
COdo si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `opencode auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/opencode/auth.json`.

```bash
COdo auth login
```

Quando COdo si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in COdo.
...
Esegui COdo in modalità non interattiva passando un prompt direttamente.
...
Puoi anche collegarti a una istanza `COdo serve` già in esecuzione per evitare i cold boot dei server MCP ad ogni esecuzione:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Attach a un server COdo in esecuzione (es. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `COdo`) |
...
Avvia un server COdo headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.

```bash
COdo serve
```

Avvia un server HTTP che espone accesso API alle funzionalità di COdo senza la TUI. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `COdo`).
...
Mostra statistiche di utilizzo token e costo per le sessioni COdo.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione COdo.

```bash
COdo import <file>
```

Puoi importare da un file locale o da un URL di condivisione COdo.
...
Avvia un server COdo headless con interfaccia web.

```bash
COdo web
```

Avvia un server HTTP e apre un browser per accedere a COdo tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `COdo`).
...
Disinstalla COdo e rimuove tutti i file correlati.
...
Aggiorna COdo all'ultima versione o a una versione specifica.
...
La CLI di COdo accetta i seguenti flag globali.
...
COdo può essere configurato tramite variabili d'ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `COdo`)     |
