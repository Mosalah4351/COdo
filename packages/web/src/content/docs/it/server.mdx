---
title: Server
description: Interagisci con il server COdo via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `COdo serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client COdo.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `COdo.local` |
...
Imposta `OPENCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `COdo`, oppure imposta `OPENCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `COdo serve` sia per `COdo web`.
...
Quando esegui `COdo` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server COdo per interagire con COdo in modo programmatico.
:::

Questa architettura permette a COdo di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `COdo serve` per avviare un server standalone. Se la TUI di COdo e gia in esecuzione, `COdo serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di COdo.
...
Il server COdo espone le seguenti API.
