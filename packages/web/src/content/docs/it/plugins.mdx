---
title: Plugin
description: Scrivi plugin per estendere COdo.
---

I plugin ti permettono di estendere COdo agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di COdo.
...
2. Config di progetto (`COdo.json`)
...
COdo esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di COdo per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere COdo.
...
:::note
Se usi l'app desktop di COdo, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a COdo di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a COdo:
...
L'helper `tool` crea uno strumento personalizzato che COdo può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in COdo insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://codo-ai.vercel.app/docs/sdk) per i dettagli.
