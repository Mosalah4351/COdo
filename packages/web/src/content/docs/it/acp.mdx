---
title: Supporto ACP
description: Usa COdo in qualunque editor compatibile con ACP.
---

COdo supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare COdo via ACP, configura l'editor in modo che esegua il comando `COdo acp`.

Il comando avvia COdo come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'COdo' nel selettore agenti di AI Chat.
...
Per usare COdo come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare COdo come agente ACP per la chat.
...
COdo funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione COdo
