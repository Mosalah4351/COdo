---
title: Regole
description: Imposta istruzioni personalizzate per COdo.
---

Puoi fornire istruzioni personalizzate a COdo creando un file `AGENTS.md`. E' simile alle regole di Cursor. Contiene istruzioni che verranno incluse nel contesto dell'LLM per personalizzarne il comportamento per il tuo progetto.

---

## Inizializza

Per creare un nuovo file `AGENTS.md`, puoi eseguire il comando `/init` in COdo.

:::tip
Dovresti committare il file `AGENTS.md` del progetto in Git.
:::

Questo scansiona il progetto e tutti i suoi contenuti per capire di cosa si tratta e generare un file `AGENTS.md`. Questo aiuta COdo a orientarsi meglio nel progetto.
...
COdo supporta anche la lettura del file `AGENTS.md` da piu' posizioni, e questo serve a scopi diversi.
...
Per gli utenti che migrano da Claude Code, COdo supporta come fallback le convenzioni di file di Claude Code:
...
Quando COdo si avvia, cerca i file di regole in questo ordine:
...
Anche se COdo non interpreta automaticamente i riferimenti a file in `AGENTS.md`, puoi ottenere una funzionalita' simile in due modi:

### Usa COdo.json

L'approccio consigliato e' usare il campo `instructions` in `COdo.json`:
...
Puoi insegnare a COdo a leggere file esterni fornendo istruzioni esplicite nel tuo `AGENTS.md`. Ecco un esempio pratico:
...
- Assicurati che COdo carichi file solo quando servono per il task specifico

:::tip
Per monorepo o progetti con standard condivisi, usare `COdo.json` con pattern glob (come `packages/*/AGENTS.md`) e' piu' manutenibile rispetto alle istruzioni manuali.
:::
