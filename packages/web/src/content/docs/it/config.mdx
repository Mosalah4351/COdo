...
description: Usa la configurazione JSON di COdo.
---

Puoi configurare COdo usando un file di configurazione JSON.

---

## Formato

COdo supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
1. **Config remota** (da `.well-known/COdo`) - default dell'organizzazione
...
4. **Config di progetto** (`COdo.json` nel progetto) - impostazioni specifiche del progetto
...
Le organizzazioni possono fornire una configurazione predefinita tramite l'endpoint `.well-known/COdo`. Viene recuperata automaticamente quando ti autentichi con un provider che lo supporta.
...
Metti la configurazione globale di COdo in `~/.config/opencode/opencode.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.
...
Aggiungi `COdo.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Quando COdo si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Il file di configurazione ha uno schema definito in [**`codo-ai.vercel.app/config.json`**](https://codo-ai.vercel.app/config.json).

La configurazione TUI usa [**`codo-ai.vercel.app/tui.json`**](https://codo-ai.vercel.app/tui.json).
...
Le chiavi legacy `theme`, `keybinds` e `tui` in `COdo.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi configurare le impostazioni del server per i comandi `COdo serve` e `COdo web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server COdo.
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `COdo.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi configurare provider e modelli da usare in COdo tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, COdo prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, COdo fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`COdo run`), app desktop e GitHub Action.
...
COdo scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
Per impostazione predefinita, COdo **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
[I plugin](/docs/plugins) estendono COdo con strumenti personalizzati, hook e integrazioni.
...
E utile quando vuoi limitare COdo a usare solo alcuni provider invece di disabilitarli uno per uno.
