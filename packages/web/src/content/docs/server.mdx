---
title: Server
description: Interact with COdo server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `COdo serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an COdo client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `COdo.local` |
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `COdo`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `COdo serve` and `COdo web`.
...
When you run `COdo` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the COdo server to interact with COdo programmatically.
:::

This architecture lets COdo support multiple clients and allows you to interact with COdo programmatically.

You can run `COdo serve` to start a standalone server. If you have the
COdo TUI running, `COdo serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the COdo [IDE](/docs/ide) plugins.
...
The COdo server exposes the following APIs.
