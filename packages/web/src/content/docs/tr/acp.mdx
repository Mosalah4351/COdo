---
title: ACP Desteği
description: COdo'u ACP uyumlu herhangi bir editörde kullanın.
---

COdo, [Agent Client Protocol](https://agentclientprotocol.com) veya (ACP) dosyalarını destekleyerek onu doğrudan uyumlu editörlerde ve IDE'lerde kullanmanızı sağlar.
...
COdo'u ACP aracılığıyla kullanmak için düzenleyicinizi `COdo acp` komutunu çalıştıracak şekilde yapılandırın.

Komut, COdo'u, editörünüzle stdio aracılığıyla JSON-RPC üzerinden iletişim kuran ACP uyumlu bir alt süreç olarak başlatır.
...
Açmak için AI Chat agent seçicisindeki yeni 'COdo' ajanını kullanın.
...
COdo'u [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim)'de ACP ajanı olarak kullanmak için Neovim yapılandırmanıza aşağıdakileri ekleyin:
...
Bu yapılandırma, CodeCompanion'ı COdo'u sohbet için ACP ajanı olarak kullanacak şekilde ayarlar.
...
COdo, terminalde olduğu gibi ACP aracılığıyla aynı şekilde çalışır. Tüm özellikler desteklenir:
...
- COdo yapılandırmanızda yapılandırılmış MCP sunucuları
