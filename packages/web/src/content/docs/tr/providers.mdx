---
title: Sağlayıcılar
description: COdo'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

COdo, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı COdo yapılandırmanızda yapılandırın.
...
Sağlayıcıları COdo'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
COdo Zen, COdo ekibi tarafından sağlanan modellerin bir listesidir.
COdo ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz COdo Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, COdo'u seçin ve [codo-ai.vercel.app/auth](https://codo-ai.vercel.app/auth)'ye gidin.
...
COdo'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## COdo Go

COdo Go, COdo ile iyi çalıştığı test edilmiş ve doğrulanmış, COdo ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `COdo Go`'yu seçin ve [codo-ai.vercel.app/auth](https://codo-ai.vercel.app/zen) adresine gidin.
...
Amazon Bedrock'u COdo ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   COdo'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `COdo.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi COdo'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
COdo'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   COdo'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
:::note[uyumluluk notu]
COdo, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. COdo'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`COdo.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [COdo-gitlab-auth](https://www.npmjs.com/package/COdo-gitlab-auth) ana sayfasında.

##### Yapılandırma

`COdo.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi COdo'la kullanmak için:
...
Google Vertex AI'yi COdo ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları COdo çalıştırılırken ayarlayın.
...
Helicone'un COdo aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her COdo görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [COdo-helicone-session](https://github.com/H2Shami/COdo-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her COdo görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
COdo'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla COdo'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla COdo'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini COdo için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/COdo)'a bakın.
:::
...
Ollama Cloud'u COdo ile kullanmak için:
...
3. COdo'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: COdo'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
COdo Zen, COdo ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>COdo Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **COdo Zen**'i arayın.

   ```txt
   /connect
   ```

3. COdo API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   COdo yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca COdo yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri COdo yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı COdo ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `COdo.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları COdo'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `COdo auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için COdo yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, COdo yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
