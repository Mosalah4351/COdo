...
Ajan becerileri, COdo'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
Proje yerel yollarinda COdo, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
...
COdo, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`COdo.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`COdo.json` icinde):
