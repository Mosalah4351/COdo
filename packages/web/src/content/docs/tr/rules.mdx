---
title: Kurallar
description: COdo için özel talimatları ayarlayın.
---

Bir `AGENTS.md` dosyası oluşturarak COdo için özel talimatlar verebilirsiniz. Bu, Cursor rules yaklaşımına benzer. Özel projeniz için davranışını özelleştirmek amacıyla LLM'in içeriğine dahil edilecek talimatları içerir.

---

## Başlat

Yeni bir `AGENTS.md` dosyası oluşturmak için `/init` komutunu COdo'da çalıştırabilirsiniz.

:::tip
Projenizin `AGENTS.md` dosyasını Git'e göndermelisiniz.
:::

Bu, projenin neyle ilgili olduğunu anlamak için projenizi ve tüm içeriğini tarayacak ve onunla bir `AGENTS.md` dosyası oluşturacaktır. Bu, COdo'un projede daha iyi gezinmesine yardımcı olur.
...
COdo ayrıca `AGENTS.md` dosyasının birden fazla konumdan okunmasını da destekler. Ve bu farklı amaçlara hizmet ediyor.
...
COdo, Claude Code'dan geçiş yapan kullanıcılar için yedek olarak Claude Code'un dosya kurallarını destekler:
...
### COdo.json'u kullanma

Önerilen yaklaşım, `instructions`'deki `COdo.json` alanını kullanmaktır:
...
`AGENTS.md`'nizde açık talimatlar sağlayarak COdo'u harici dosyaları okumayı öğretebilirsiniz. İşte pratik bir örnek:
...
- COdo'un dosyaları yalnızca belirli bir görev için gerektiğinde yüklediğinden emin olun

:::tip
Monorepo'lar veya ortak standartlara sahip projeler için, `COdo.json`'yi glob desenleriyle (`packages/*/AGENTS.md` gibi) kullanmak, manuel talimatlara göre daha sürdürülebilirdir.
:::
