...
_Model Bağlam Protokolü_ veya MCP kullanarak COdo'a harici araçlar ekleyebilirsiniz. COdo hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [COdo Config](https://codo-ai.vercel.app/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/COdo` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
COdo, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde COdo şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa COdo, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `COdo mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
MCP'leriniz yerleşik araçların yanı sıra COdo'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi COdo yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve COdo'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
