...
COdo Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, COdo'daki diğer sağlayıcılar gibi çalışır. COdo Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve COdo'u kullanmak için bunu kullanmanıza gerek yoktur.
...
COdo Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

COdo Go, COdo'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>COdo Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `COdo Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye COdo Go'ya abone olabilir.
:::
...
COdo Go aşağıdaki limitleri içerir:
...
COdo yapılandırmanızdaki [model id](/docs/config/#models) formatı `COdo-go/<model-id>` şeklindedir. Örneğin, Kimi K2.7 Code için yapılandırmanızda `COdo-go/kimi-k2.7-code` kullanmalısınız.
...
COdo Go'yu şu amaçlarla oluşturduk:
...
4. COdo ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
