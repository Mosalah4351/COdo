---
title: TUI
description: COdo terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

COdo, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

COdo'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
COdo TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
COdo'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
COdo'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `COdo.json` dosyasından ayrıdır.
...
- `leader_timeout` - COdo'un leader key sonrasında ne kadar bekleyeceğini kontrol eder. Varsayılan değer `2000`.
...
- `sound_pack` - Kullanılacak sound pack ID. Varsayılan değer `COdo.default`.
