---
title: TUI
description: Usando a interface de usuário de terminal COdo.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

O COdo fornece uma interface de terminal interativa ou TUI para trabalhar em seus projetos com um LLM.

Executar COdo inicia o TUI para o diretório atual.
...
Ao usar o TUI do COdo, você pode digitar `/` seguido pelo nome de um comando para executar ações rapidamente. Por exemplo:
...
Adicione um provedor ao COdo. Permite que você selecione entre os provedores disponíveis e adicione suas chaves de API.
...
Isso é separado do `COdo.json`, que configura o comportamento do servidor/runtime.
...
- `leader_timeout` - Controla por quanto tempo o COdo espera depois da leader key. O padrão é `2000`.
...
- `sound_pack` - ID do sound pack a ser usado. O padrão é `COdo.default`.
