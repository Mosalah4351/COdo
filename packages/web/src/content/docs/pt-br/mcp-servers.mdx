...
Você pode adicionar ferramentas externas ao COdo usando o _Model Context Protocol_, ou MCP. O COdo suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do COdo](https://codo-ai.vercel.app/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/COdo`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O COdo lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o COdo irá:
...
Se o servidor requer autenticação, o COdo solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `COdo mcp auth <server-name>`.
...
Seus MCPs estão disponíveis como ferramentas no COdo, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do COdo como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o COdo à sua conta do Sentry.
