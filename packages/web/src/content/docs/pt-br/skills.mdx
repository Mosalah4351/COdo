...
As habilidades do agente permitem que o COdo descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
Para caminhos locais do projeto, o COdo sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
...
O COdo lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `COdo.json`:
...
**Para agentes embutidos** (em `COdo.json`):
