---
title: Servidor
description: Interaja com o servidor COdo via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `COdo serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente COdo pode usar.
...
```bash
COdo serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `COdo.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
COdo serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `OPENCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `COdo`, ou defina `OPENCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `COdo serve` quanto ao `COdo web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password COdo serve
```
...
Quando você executa `COdo`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor COdo para interagir com o COdo programaticamente.
:::

Essa arquitetura permite que o COdo suporte múltiplos clientes e permite que você interaja com o COdo programaticamente.

Você pode executar `COdo serve` para iniciar um servidor autônomo. Se você tiver o TUI do COdo em execução, `COdo serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do COdo [IDE](/docs/ide).
...
O servidor COdo expõe as seguintes APIs.
