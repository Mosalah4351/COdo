---
title: Plugins
description: Escreva seus próprios plugins para estender o COdo.
---

Plugins permitem que você estenda o COdo conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do COdo.
...
2. Configuração do projeto (`COdo.json`)
...
O COdo executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do COdo para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o COdo.
...
:::note
Se você estiver usando o aplicativo desktop COdo, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o COdo de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao COdo:
...
O helper `tool` cria uma ferramenta personalizada que o COdo pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o COdo junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://codo-ai.vercel.app/docs/sdk) para detalhes.
