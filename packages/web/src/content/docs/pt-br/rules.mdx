---
title: Regras
description: Defina instruções personalizadas para COdo.
---

Você pode fornecer instruções personalizadas para COdo criando um arquivo `AGENTS.md`. Isso é semelhante às regras do Cursor. Ele contém instruções que serão incluídas no contexto do LLM para personalizar seu comportamento para o seu projeto específico.

---

## Inicializar

Para criar um novo arquivo `AGENTS.md`, você pode executar o comando `/init` no COdo.

:::tip
Você deve commitar o arquivo `AGENTS.md` do seu projeto no Git.
:::

Isso irá escanear seu projeto e todo o seu conteúdo para entender do que se trata o projeto e gerar um arquivo `AGENTS.md` com isso. Isso ajuda o COdo a navegar melhor pelo projeto.
...
O COdo também suporta a leitura do arquivo `AGENTS.md` de múltiplos locais. E isso serve a diferentes propósitos.
...
Para usuários migrando do Claude Code, o COdo suporta as convenções de arquivo do Claude Code como alternativas:
...
Quando o COdo inicia, ele procura arquivos de regras nesta ordem:
...
Embora o COdo não analise automaticamente referências de arquivos em `AGENTS.md`, você pode alcançar funcionalidade semelhante de duas maneiras:

### Usando COdo.json

A abordagem recomendada é usar o campo `instructions` em `COdo.json`:
...
Você pode ensinar o COdo a ler arquivos externos fornecendo instruções explícitas em seu `AGENTS.md`. Aqui está um exemplo prático:
...
- Garanta que o COdo carregue arquivos apenas quando necessário para a tarefa específica

:::tip
Para monorepos ou projetos com padrões compartilhados, usar `COdo.json` com padrões glob (como `packages/*/AGENTS.md`) é mais sustentável do que instruções manuais.
:::
