...
O COdo Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no COdo. Você assina o COdo Go e
...
O COdo Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O COdo Go funciona como qualquer outro provedor no COdo.

1. Você entra no **<a href={console}>COdo Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `COdo Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o COdo Go.
:::
...
O COdo Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do COdo
usa o formato `COdo-go/<model-id>`. Por exemplo, para o Kimi K2.7 Code, você usaria
`COdo-go/kimi-k2.7-code` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o COdo também.
