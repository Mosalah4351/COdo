---
title: Configuração
description: Usando a configuração JSON do COdo.
---

Você pode configurar o COdo usando um arquivo de configuração JSON.

---

## Formato

O COdo suporta os formatos **JSON** e **JSONC** (JSON com Comentários).
...
1. **Configuração remota** (de `.well-known/COdo`) - padrões organizacionais
...
4. **Configuração do projeto** (`COdo.json` no projeto) - configurações específicas do projeto
...
As organizações podem fornecer configuração padrão através do endpoint `.well-known/COdo`. Isso é buscado automaticamente quando você se autentica com um provedor que o suporta.
...
Adicione `COdo.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Quando o COdo é iniciado, ele procura um arquivo de configuração no diretório atual ou sobe até o diretório Git mais próximo.
...
O esquema de configuração do servidor/tempo de execução é definido em [**`codo-ai.vercel.app/config.json`**](https://codo-ai.vercel.app/config.json).

A configuração do TUI usa [**`codo-ai.vercel.app/tui.json`**](https://codo-ai.vercel.app/tui.json).
...
Chaves legadas `theme`, `keybinds` e `tui` em `COdo.json` estão obsoletas e são migradas automaticamente quando possível.
...
Você pode configurar as configurações do servidor para os comandos `COdo serve` e `COdo web` através da opção `server`.
...
- `mdns` - Habilitar descoberta de serviço mDNS. Isso permite que outros dispositivos na rede descubram seu servidor COdo.
- `mdnsDomain` - Nome de domínio personalizado para o serviço mDNS. O padrão é `COdo.local`. Útil para executar várias instâncias na mesma rede.
...
Você pode configurar os provedores e modelos que deseja usar em sua configuração do COdo através das opções `provider`, `model` e `small_model`.
...
A opção `small_model` configura um modelo separado para tarefas leves, como geração de títulos. Por padrão, o COdo tenta usar um modelo mais barato se um estiver disponível do seu provedor, caso contrário, ele recua para seu modelo principal.
...
O agente padrão deve ser um agente primário (não um subagente). Isso pode ser um agente embutido como `"build"` ou `"plan"`, ou um [agente personalizado](/docs/agents) que você definiu. Se o agente especificado não existir ou for um subagente, o COdo recuará para `"build"` com um aviso.

Essa configuração se aplica a todas as interfaces: TUI, CLI (`COdo run`), aplicativo desktop e GitHub Action.
...
O COdo fará o download automaticamente de quaisquer novas atualizações quando for iniciado. Você pode desabilitar isso com a opção `autoupdate`.
...
Por padrão, o COdo **permite todas as operações** sem exigir aprovação explícita. Você pode mudar isso usando a opção `permission`.
...
[Plugins](/docs/plugins) estendem o COdo com ferramentas, hooks e integrações personalizadas.
...
Isso é útil quando você deseja restringir o COdo para usar apenas provedores específicos, em vez de desabilitá-los um a um.
