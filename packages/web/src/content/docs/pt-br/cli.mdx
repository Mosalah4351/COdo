---
title: CLI
description: Opções e comandos da CLI do COdo.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

A CLI do COdo, por padrão, inicia o [TUI](/docs/tui) quando executada sem argumentos.

```bash
COdo
```

Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o COdo programaticamente.
...
Inicie a interface de usuário do terminal do COdo.
...
A CLI do COdo também possui os seguintes comandos.

---

### agent

Gerencie agentes para o COdo.
...
Anexe um terminal a um servidor backend do COdo já em execução, iniciado via comandos `serve` ou `web`.

```bash
COdo attach [url]
```

Isso permite usar o TUI com um backend COdo remoto. Por exemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `COdo`) |
...
Quando o COdo é iniciado, ele carrega os provedores do arquivo de credenciais. E se houver chaves definidas em seus ambientes ou em um arquivo `.env` em seu projeto.
...
Use a flag `--refresh` para atualizar a lista de modelos em cache. Isso é útil quando novos modelos foram adicionados a um provedor e você deseja vê-los no COdo.
...
Execute o COdo em modo não interativo passando um prompt diretamente.
...
```bash "COdo run"
COdo run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `COdo serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
COdo run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Anexe a um servidor COdo em execução (por exemplo, http://localhost:4096)     |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `COdo`) |
...
Inicie um servidor COdo sem cabeça para acesso à API. Confira a [documentação do servidor](/docs/server) para a interface HTTP completa.

```bash
COdo serve
```

Isso inicia um servidor HTTP que fornece acesso à funcionalidade do COdo sem a interface TUI. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `COdo`).
...
Liste todas as sessões do COdo.
...
Mostre o uso de tokens e estatísticas de custo para suas sessões do COdo.
...
Importe dados da sessão de um arquivo JSON ou URL de compartilhamento do COdo.

```bash
COdo import <file>
```

Você pode importar de um arquivo local ou de uma URL de compartilhamento do COdo.
...
Inicie um servidor COdo sem cabeça com uma interface web.

```bash
COdo web
```

Isso inicia um servidor HTTP e abre um navegador para acessar o COdo através de uma interface web. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `COdo`).
...
Desinstale o COdo e remova todos os arquivos relacionados.
...
Atualiza o COdo para a versão mais recente ou uma versão específica.
...
A CLI do COdo aceita as seguintes flags globais.
...
O COdo pode ser configurado usando variáveis de ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `COdo`) |
