...
Para depurar problemas com o COdo, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\COdo\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `COdo --log-level DEBUG`.

---

## Armazenamento

O COdo armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\COdo`
...
O COdo Desktop executa um servidor COdo local (o sidecar `COdo-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `COdo` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\COdo\COdo.jsonc`
...
O COdo também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\COdo\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o COdo possa reconstruí-lo.

1. Saia completamente do COdo Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/COdo`
- **Linux**: exclua `~/.cache/COdo` (ou execute `rm -rf ~/.cache/COdo`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\COdo`

3. Reinicie o COdo Desktop.

---

### Corrigindo problemas de conexão com o servidor

O COdo Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `COdo.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.
...
No Windows, o COdo Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do COdo.

---

### Notificações não aparecendo

O COdo Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o COdo nas configurações do seu sistema operacional, e
...
1. Saia do COdo Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo COdo Desktop):

- `COdo.settings.dat` (URL do servidor padrão do desktop)
- `COdo.global.dat` e `COdo.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o COdo:
...
3. Certifique-se de que você tem a versão mais recente com `COdo upgrade`
...
Para descobrir quais modelos você tem acesso, execute `COdo models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\COdo`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O COdo instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\COdo`

2. Reinicie o COdo para reinstalar os pacotes de provedores mais recentes

Isso forçará o COdo a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O COdo detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
