---
title: المزوّدون
description: استخدام أي موفّر LLM في COdo.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم COdo ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات COdo.
...
يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات COdo.
...
COdo Zen هي قائمة نماذج يوفّرها فريق COdo وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع COdo. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع COdo Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر COdo، ثم انتقل إلى [codo-ai.vercel.app/auth](https://codo-ai.vercel.app/auth).
...
يعمل مثل أي مزوّد آخر في COdo واستخدامه اختياري بالكامل.

---

## COdo Go

COdo Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق COdo، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع COdo.

1. شغّل الأمر `/connect` في TUI، واختر `COdo Go`، ثم انتقل إلى [codo-ai.vercel.app/auth](https://codo-ai.vercel.app/zen).
...
لاستخدام Amazon Bedrock مع COdo:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل COdo:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `COdo.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل COdo غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين COdo لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل COdo بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات COdo.
...
تخزين الرمز في مساحة تخزين مصادقة COdo.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم COdo نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر COdo
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`COdo.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [COdo-gitlab-auth](https://www.npmjs.com/package/COdo-gitlab-auth).
...
لاستخدام اشتراك GitHub Copilot مع COdo:
...
لاستخدام Google Vertex AI مع COdo:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر COdo، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [COdo-helicone-session](https://github.com/H2Shami/COdo-helicone-session) لتسجيل كل محادثة COdo تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة COdo مدرجة كجلسة مستقلة.
...
يمكنك ضبط COdo لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط COdo لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط COdo لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع COdo. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/COdo) للتفاصيل.
:::
...
3. انسخ مفتاح API لاستخدامه في COdo.
...
6. **مهم**: قبل استخدام نماذج السحابة في COdo، يجب سحب معلومات النموذج محليا:
...
COdo Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق COdo. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>COdo Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **COdo Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ COdo.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات COdo.
...
5. يمكنك أيضا تخصيصها عبر إعدادات COdo. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات COdo. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع COdo. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `COdo.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ COdo بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `COdo auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات COdo وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات COdo.
