---
title: القواعد
description: اضبط تعليمات مخصصة لـ COdo.
---

يمكنك تزويد COdo بتعليمات مخصصة عبر إنشاء ملف `AGENTS.md`. هذا مشابه لقواعد Cursor. يحتوي هذا الملف على تعليمات تُضاف إلى سياق نموذج اللغة (LLM) لتخصيص سلوكه لمشروعك المحدد.

---

## التهيئة

لإنشاء ملف `AGENTS.md` جديد، يمكنك تشغيل الأمر `/init` في COdo.

:::tip
يُستحسن عمل commit لملف `AGENTS.md` الخاص بمشروعك إلى Git.
:::

سيقوم ذلك بفحص مشروعك وكل محتوياته لفهم طبيعة المشروع وتوليد ملف `AGENTS.md` بناءً على ذلك. يساعد هذا COdo على التنقّل داخل المشروع بشكل أفضل.
...
يدعم COdo أيضًا قراءة ملف `AGENTS.md` من مواقع متعددة، ولكل موقع غرض مختلف.
...
للمستخدمين الذين ينتقلون من Claude Code، يدعم COdo اصطلاحات ملفات Claude Code كبدائل احتياطية:
...
عند بدء COdo، يبحث عن ملفات القواعد بهذا الترتيب:
...
على الرغم من أن COdo لا يقوم تلقائيًا بتحليل مراجع الملفات داخل `AGENTS.md`، يمكنك تحقيق وظيفة مشابهة بطريقتين:

### استخدام COdo.json

النهج الموصى به هو استخدام الحقل `instructions` في `COdo.json`:
...
يمكنك تعليم COdo قراءة ملفات خارجية عبر تقديم تعليمات صريحة داخل `AGENTS.md`. إليك مثالًا عمليًا:
...
- ضمان أن COdo يحمّل الملفات فقط عند الحاجة للمهمة المحددة

:::tip
بالنسبة للمستودعات متعددة الحزم (monorepos) أو المشاريع ذات المعايير المشتركة، فإن استخدام `COdo.json` مع أنماط glob (مثل `packages/*/AGENTS.md`) أسهل للصيانة من التعليمات اليدوية.
:::
