---
title: الخادم
description: تفاعل مع خادم COdo عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `COdo serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل COdo استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `COdo.local` |
...
عيّن `OPENCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `COdo`، أو عيّن `OPENCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `COdo serve` و `COdo web`.
...
عند تشغيل `COdo` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم COdo للتفاعل مع COdo برمجيا.
:::

تتيح هذه البنية لـ COdo دعم عدة عملاء وتمكّنك من التفاعل مع COdo برمجيا.

يمكنك تشغيل `COdo serve` لبدء خادم مستقل. إذا كانت واجهة COdo في terminal (TUI)
قيد التشغيل، فسيبدأ `COdo serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات COdo لـ [IDE](/docs/ide).
...
يعرض خادم COdo واجهات API التالية.
