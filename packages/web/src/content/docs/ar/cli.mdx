---
title: CLI
description: أوامر وخيارات CLI في COdo.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يبدأ CLI الخاص بـ COdo افتراضيا واجهة [TUI](/docs/tui) عند تشغيله بدون أي معاملات.

```bash
COdo
```

كما يدعم أيضا أوامر موثقة في هذه الصفحة، ما يتيح لك التفاعل مع COdo برمجيا.
...
بدء واجهة المستخدم terminal (TUI) الخاصة بـ COdo.
...
يتضمن CLI الخاص بـ COdo أيضا الأوامر التالية.
...
إرفاق terminal بخادم الواجهة الخلفية لـ COdo قيد التشغيل بالفعل، والذي تم تشغيله عبر الأمرين `serve` أو `web`.

```bash
COdo attach [url]
```

يتيح ذلك استخدام واجهة TUI مع واجهة خلفية لـ COdo تعمل عن بعد. على سبيل المثال:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `COdo`) |
...
يعتمد COdo على قائمة المزودين في [Models.dev](https://models.dev)، لذا يمكنك استخدام `opencode auth login` لتهيئة مفاتيح API لأي مزود ترغب باستخدامه. يتم حفظ ذلك في `~/.local/share/opencode/auth.json`.

```bash
COdo auth login
```

عند تشغيل COdo يقوم بتحميل المزودين من ملف بيانات الاعتماد، وكذلك أي مفاتيح معرّفة في متغيرات البيئة لديك أو في ملف `.env` ضمن مشروعك.
...
استخدم الراية `--refresh` لتحديث قائمة النماذج المخزنة مؤقتا. يفيد ذلك عند إضافة نماذج جديدة إلى مزود وتريد رؤيتها في COdo.
...
تشغيل COdo في وضع غير تفاعلي عبر تمرير موجه مباشرة.
...
يمكنك أيضا الإرفاق بمثيل `COdo serve` قيد التشغيل لتجنّب زمن الإقلاع البارد لخوادم MCP في كل تشغيل:
...
| <nobr><code>{"--attach"}</code></nobr>   |         | الإرفاق بخادم COdo قيد التشغيل (مثل http://localhost:4096)                    |
| <nobr><code>{"--password"}</code></nobr> | `-p`    | كلمة مرور المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_PASSWORD`)                |
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `COdo`) |
...
بدء خادم COdo بدون واجهة للوصول عبر API. راجع [توثيق الخادم](/docs/server) لواجهة HTTP الكاملة.

```bash
COdo serve
```

يشغّل هذا خادم HTTP يوفّر وصولا عبر API لوظائف COdo دون واجهة TUI. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `COdo`).
...
عرض إحصاءات استخدام الرموز والتكلفة لجلسات COdo لديك.
...
استيراد بيانات الجلسة من ملف JSON أو رابط مشاركة COdo.

```bash
COdo import <file>
```

يمكنك الاستيراد من ملف محلي أو من رابط مشاركة COdo.
...
بدء خادم COdo بدون واجهة مع واجهة ويب.

```bash
COdo web
```

يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى COdo عبر واجهة ويب. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `COdo`).
...
إلغاء تثبيت COdo وإزالة جميع الملفات ذات الصلة.
...
تحديث COdo إلى أحدث إصدار أو إلى إصدار محدد.
...
يدعم سطر أوامر COdo الخيارات العامة التالية.
...
يمكن تهيئة COdo باستخدام متغيرات البيئة.
...
| `OPENCODE_SERVER_USERNAME`            | string  | تجاوز اسم مستخدم المصادقة الأساسية (الافتراضي `COdo`) |
