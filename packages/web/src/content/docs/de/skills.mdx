...
Agent Skills erlauben COdo, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
COdo sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft COdo vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
...
COdo listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `COdo.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `COdo.json`):
