---
title: Regeln
description: Lege eigene Anweisungen fuer COdo fest.
---

Du kannst COdo ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in COdo den Befehl `/init` aus.
...
So kann COdo sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht COdo sie zu erweitern.
...
COdo kann `AGENTS.md` aus mehreren Orten lesen.
...
Diese gelten in allen COdo-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt COdo dessen Dateikonventionen als Fallback:
...
Beim Start sucht COdo in dieser Reihenfolge nach Regeldateien:
...
COdo parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit COdo.json

Empfohlen ist das `instructions`-Feld in der `COdo.json`:
...
Du kannst COdo explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `COdo.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
