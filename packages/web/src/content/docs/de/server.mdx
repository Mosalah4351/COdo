---
title: Server
description: Nutze den COdo-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `COdo serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein COdo-Client nutzen kann.
...
Der Benutzername ist standardmaessig `COdo`, kann aber mit `OPENCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `COdo serve` und `COdo web`.
...
Wenn du `COdo` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den COdo-Server, um COdo programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `COdo serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `COdo serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die COdo-[IDE](/docs/ide)-Plugins.
...
Der COdo-Server stellt folgende APIs bereit.
