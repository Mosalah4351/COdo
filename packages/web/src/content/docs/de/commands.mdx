...
Sie können benutzerdefinierte Befehle über die COdo-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem COdo [config](/docs/config):

```json title="COdo.jsonc" {4-12}
{
...
COdo enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
