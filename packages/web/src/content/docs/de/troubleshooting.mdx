...
Wenn COdo Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\COdo\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `COdo --log-level DEBUG`.

---

## Speicher

COdo speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\COdo`
...
COdo Desktop startet im Hintergrund einen lokalen COdo-Server (`COdo-cli`-Sidecar).
...
- Nur macOS: `COdo`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\COdo\COdo.jsonc`
...
COdo kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\COdo\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit COdo ihn neu aufbauen kann.

1. Beende COdo Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/COdo`
- **Linux**: loesche `~/.cache/COdo` (oder fuehre aus: `rm -rf ~/.cache/COdo`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\COdo`

3. Starte COdo Desktop neu.

---

### Server-Verbindungsprobleme beheben

COdo Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `COdo.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.
...
Unter Windows benoetigt COdo Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit COdo-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

COdo Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer COdo in den OS-Einstellungen aktiviert sind, und
...
1. Beende COdo Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von COdo Desktop):

- `COdo.settings.dat` (desktop default server URL)
- `COdo.global.dat` und `COdo.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit COdo hast:
...
3. Stelle sicher, dass du die neueste Version hast: `COdo upgrade`
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `COdo models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\COdo`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. COdo installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\COdo`

2. Starte COdo neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt COdo, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
COdo erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
