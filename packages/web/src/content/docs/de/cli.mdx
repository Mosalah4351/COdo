---
title: CLI
description: COdo CLI Optionen und Befehle.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Der COdo CLI startet standardmäßig das [TUI](/docs/tui), wenn er ohne Argumente ausgeführt wird.

```bash
COdo
```

Es akzeptiert aber auch Befehle, wie auf dieser Seite dokumentiert. Dadurch können Sie programmgesteuert mit COdo interagieren.
...
Starten Sie die Terminalbenutzeroberfläche COdo.
...
Der COdo CLI verfügt außerdem über die folgenden Befehle.

---

### agent

Agenten für COdo verwalten.
...
Schließen Sie ein Terminal an einen bereits laufenden COdo-Backend-Server an, der über die Befehle `serve` oder `web` gestartet wurde.

```bash
COdo attach [url]
```

Dies ermöglicht die Verwendung von TUI mit einem Remote-Backend COdo. Zum Beispiel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `COdo`) |
...
COdo wird von der Anbieterliste bei [Models.dev](https://models.dev) unterstützt, sodass Sie `opencode auth login` verwenden können, um API-Schlüssel für jeden Anbieter zu konfigurieren, den Sie verwenden möchten. Dies wird in `~/.local/share/opencode/auth.json` gespeichert.

```bash
COdo auth login
```

Wenn COdo startet, lädt es den Anbieter aus der Anmeldeinformationsdatei. Und ob in Ihren Umgebungen Schlüssel sind oder in Ihrem Projekt eine `.env`-Datei vorhanden ist.
...
Verwenden Sie das Flag `--refresh`, um die zwischengespeicherte Modellliste zu aktualisieren. Dies ist nützlich, wenn einem Anbieter neue Modelle hinzugefügt wurden und Sie diese in COdo sehen möchten.
...
Führen Sie COdo im nicht interaktiven Modus aus, indem Sie direkt eine Eingabeaufforderung übergeben.
...
Sie können auch eine Verbindung zu einer laufenden `COdo serve`-Instanz herstellen, um bei jedem Lauf Kaltstartzeiten für den MCP-Server zu vermeiden:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | An einen laufenden COdo-Server anschließen (z.B. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Basic-Auth-Passwort (standardmäßig `OPENCODE_SERVER_PASSWORD`)                                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `COdo`)                  |
...
Starten Sie einen Headless-COdo-Server für den API-Zugriff. Sehen Sie sich die [Server-Dokumentation](/docs/server) für die vollständige HTTP-Schnittstelle an.

```bash
COdo serve
```

Dadurch wird ein HTTP-Server gestartet, der API-Zugriff auf COdo-Funktionalität ohne die TUI-Schnittstelle bietet. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `COdo`).
...
Listen Sie alle COdo-Sitzungen auf.
...
Zeigen Sie Token-Nutzungs- und Kostenstatistiken für Ihre COdo-Sitzungen an.
...
Importieren Sie Sitzungsdaten aus einer JSON-Datei oder einer COdo-Freigabe-URL.

```bash
COdo import <file>
```

Sie können aus einer lokalen Datei oder einer COdo-Freigabe-URL importieren.
...
Starten Sie einen Headless-COdo-Server mit einer Weboberfläche.

```bash
COdo web
```

Dadurch wird ein HTTP-Server gestartet und ein Webbrowser geöffnet, um über eine Webschnittstelle auf COdo zuzugreifen. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `COdo`).
...
Deinstallieren Sie COdo und entfernen Sie alle zugehörigen Dateien.
...
Aktualisiert COdo auf die neueste Version oder eine bestimmte Version.
...
Der COdo CLI akzeptiert die folgenden globalen Flags.
...
COdo kann mithilfe von Umgebungsvariablen konfiguriert werden.
...
| `OPENCODE_SERVER_USERNAME`            | Zeichenfolge    | Benutzernamen für die Basisauthentifizierung überschreiben (Standard `COdo`) |
