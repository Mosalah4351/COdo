---
title: Policies
description: Control which configured resources COdo may use.
---

Policies control whether COdo may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `COdo.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether COdo may use a resource such as an LLM provider.
...
COdo currently supports one policy action:
