...
Umiejętności agenta pozwalają COdo odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
COdo przeszukuje te lokalizacje:
...
W przypadku ścieżek lokalnych projektu COdo przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
...
COdo wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `COdo.json`:
...
**Dla agentów wbudowanych** (w `COdo.json`):
