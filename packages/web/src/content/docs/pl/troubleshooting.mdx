...
Aby debugować problemy z COdo, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\COdo\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `COdo --log-level DEBUG`.

---

## Przechowywanie danych

COdo przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\COdo`
...
COdo Desktop uruchamia w tle lokalny serwer COdo (sidecar `COdo-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `COdo` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\COdo\COdo.jsonc`
...
COdo może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\COdo\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby COdo mógł ją odbudować.

1. Całkowicie zamknij COdo Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/COdo`
- **Linux**: usuń `~/.cache/COdo` (lub uruchom `rm -rf ~/.cache/COdo`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\COdo`

3. Uruchom ponownie COdo Desktop.

---

### Napraw problemy z połączeniem z serwerem

COdo Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `COdo.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.
...
W systemie Windows COdo Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami COdo.

---

### Brak powiadomień

COdo Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla COdo w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie COdo Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji COdo Desktop):

- `COdo.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `COdo.global.dat` i `COdo.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `COdo upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `COdo models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\COdo`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. COdo dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\COdo`

2. Uruchom ponownie kod COdo, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to COdo do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
COdo wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
