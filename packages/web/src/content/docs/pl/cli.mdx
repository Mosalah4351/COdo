---
title: CLI
description: Opcje i polecenia wiersza poleceń COdo.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI COdo domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
COdo
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać COdo programowo.
...
Uruchamia interfejs terminalowy (TUI) COdo.
...
Interfejs CLI COdo zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami COdo.
...
Dołącz terminal do już działającego serwera backendu COdo uruchomionego za pomocą `serve` lub `web`.

```bash
COdo attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem COdo. Na przykład:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `COdo`) |
...
COdo integruje się z wieloma dostawcami. Możesz użyć `opencode auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/opencode/auth.json`.

```bash
COdo auth login
```

Podczas uruchamiania COdo ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w COdo.
...
Uruchom COdo w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
Można również użyć tego z działającym `COdo serve`, aby uniknąć zimnego startu serwera MCP przy każdym uruchomieniu:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Dołącz do działającego serwera COdo (np. http://localhost:4096)                                   |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Hasło uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_PASSWORD`)                            |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `COdo`) |
...
Uruchom serwer COdo (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
COdo serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API COdo bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `COdo`).
...
Pokaż statystyki wykorzystania tokenów i sesji COdo.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania COdo.
...
Uruchom serwer COdo z interfejsem internetowym.

```bash
COdo web
```

Uruchamia to serwer HTTP i udostępnia COdo przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `COdo`).
...
Odinstaluj COdo i usuń wszystkie powiązane pliki.
...
Aktualizuj COdo do najnowszej lub określonej wersji.
...
Interfejs CLI COdo przyjmuje następujące flagi globalne dla każdego polecenia.
...
COdo można skonfigurować za pomocą zmiennych środowiskowych.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Nazwa użytkownika do autoryzacji (domyślnie `COdo`)    |
