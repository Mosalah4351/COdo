---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w COdo.
---

import config from "../../../../config.mjs"
export const console = config.console

COdo używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji COdo.
...
Możesz dostosować dostawców za pomocą sekcji `provider` w swoim COdo
config.
...
COdo Zen to lista modeli dostarczonych przez zespół COdo, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z COdo. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od COdo Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `COdo Zen` i przejdź do [codo-ai.vercel.app/auth](https://codo-ai.vercel.app/zen).
...
Działa jak każdy inny dostawca w COdo i jest całkowicie opcjonalny w użyciu.

---

## COdo Go

COdo Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół COdo, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z COdo.

1. Uruchom polecenie `/connect` w TUI, wybierz `COdo Go` i przejdź do [codo-ai.vercel.app/auth](https://codo-ai.vercel.app/zen).
...
Aby używać Amazon Bedrock z COdo:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania COdo:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `COdo.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w COdo nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować COdo do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod COdo działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji COdo.
...
COdo używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować COdo
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`COdo.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [COdo-gitlab-auth](https://www.npmjs.com/package/COdo-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem COdo:
...
Aby używać Google Vertex AI z COdo:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania COdo.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą COdo, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [COdo-helicone-session](https://github.com/H2Shami/COdo-helicone-session), aby automatycznie rejestrować każdą konwersację COdo jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja COdo będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować COdo tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować COdo tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować COdo tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem COdo. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/COdo).
:::
...
Aby korzystać z Ollama Cloud z COdo:
...
3. Skopiuj klucz API do użycia w COdo.
...
6. **Ważne**: Przed użyciem modeli chmurowych w COdo musisz pobrać informacje o modelu lokalnie:
...
COdo Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół COdo. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>COdo Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **COdo Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API COdo.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji COdo.
...
5. Możesz je także dostosować za pomocą konfiguracji COdo. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z COdo:
...
Możesz także dostosować modele za pomocą konfiguracji COdo. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z COdo. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `COdo.json` w katalogu projektu:
...
Pola `limit` pozwalają COdo zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `COdo auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację COdo i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji COdo.
