---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć COdo.
---

Wtyczki umożliwiające wyłączenie COdo poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania COdo.
...
2. Konfiguracja projektu (`COdo.json`)
...
COdo uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z COdo do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do COdo.
...
:::note
Jeśli korzystasz z aplikacji komputerowej COdo, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj COdo czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do COdo:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod COdo. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://codo-ai.vercel.app/docs/sdk).
