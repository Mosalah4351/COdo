---
title: Serwer
description: Interakcja z serwerem COdo za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `COdo serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient COdo.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `COdo.local` |
...
Ustaw `OPENCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `COdo` lub ustaw `OPENCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `COdo serve`, jak i `COdo web`.
...
Kiedy uruchomisz `COdo`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera COdo do programowej interakcji z COdo.
:::

Ta architektura umożliwia obsługę wielu klientów przez COdo i programową interakcję z kodem otwartym.

Możesz uruchomić `COdo serve`, aby uruchomić samodzielny serwer. Jeśli masz
COdo TUI działa, `COdo serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki COdo [IDE](/docs/ide).
...
Serwer COdo udostępnia następujące interfejsy API.
