...
Możesz dodać dodatkowe informacje do COdo za pomocą _Model Context Protocol_, w skrócie MCP. COdo obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [COdo Config](https://codo-ai.vercel.app/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/COdo`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
COdo automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, COdo:
...
Jeśli serwer wymaga uwierzytelnienia, COdo poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `COdo mcp auth <server-name>`.
...
Twoje MCP są dostępne jako narzędzie w COdo, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie COdo, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć COdo z kontem Sentry.
