---
title: 规则
description: 为 COdo 设置自定义指令。
---

您可以通过创建 `AGENTS.md` 文件来为 COdo 提供自定义指令。这类似于 Cursor 的规则功能。该文件包含的指令会被纳入 LLM 的上下文中，以便针对您的特定项目自定义其行为。

---

## 初始化

要创建新的 `AGENTS.md` 文件，您可以在 COdo 中运行 `/init` 命令。

:::tip
您应该将项目的 `AGENTS.md` 文件提交到 Git。
:::

该命令会扫描您的项目及其所有内容，了解项目的用途，并据此生成一个 `AGENTS.md` 文件。这有助于 COdo 更好地导航您的项目。
...
COdo 还支持从多个位置读取 `AGENTS.md` 文件，不同的位置有不同的用途。
...
对于从 Claude Code 迁移过来的用户，COdo 支持 Claude Code 的文件约定作为回退方案：
...
虽然 COdo 不会自动解析 `AGENTS.md` 中的文件引用，但您可以通过以下两种方式实现类似的功能：

### 使用 COdo.json

推荐的方式是使用 `COdo.json` 中的 `instructions` 字段：
...
您可以在 `AGENTS.md` 中提供明确的指令，教 COdo 读取外部文件。以下是一个实际示例：
...
- 确保 COdo 仅在特定任务需要时才加载文件

:::tip
对于 monorepo 或具有共享标准的项目，使用 `COdo.json` 配合 glob 模式（如 `packages/*/AGENTS.md`）比手动指定指令更易于维护。
:::
