---
title: 配置
description: 使用 COdo JSON 配置。
---

您可以使用 JSON 配置文件来配置 COdo。

---

## 格式

COdo 支持 **JSON** 和 **JSONC**（带注释的 JSON）格式。
...
组织可以通过 `.well-known/COdo` 端点提供默认配置。当您使用支持该功能的提供商进行身份验证时，会自动获取此配置。
...
将全局 COdo 配置放在 `~/.config/opencode/opencode.json` 中。使用全局配置来设置用户级别的偏好，例如主题、提供商或快捷键。
...
在项目根目录中添加 `COdo.json`。项目配置在标准配置文件中具有最高优先级——它会覆盖全局配置和远程配置。

:::tip
将项目特定配置放在项目的根目录中。
:::

当 COdo 启动时，它会在当前目录中查找配置文件，或向上遍历到最近的 Git 目录。
...
配置文件具有在 [**`codo-ai.vercel.app/config.json`**](https://codo-ai.vercel.app/config.json) 中定义的 Schema。
...
您可以通过 `server` 选项为 `COdo serve` 和 `COdo web` 命令配置服务器设置。
...
- `mdns` - 启用 mDNS 服务发现。这允许网络上的其他设备发现您的 COdo 服务器。
- `mdnsDomain` - mDNS 服务的自定义域名。默认为 `COdo.local`。适用于在同一网络上运行多个实例的场景。
...
您可以通过 `provider`、`model` 和 `small_model` 选项在 COdo 配置中设置要使用的提供商和模型。
...
`small_model` 选项为标题生成等轻量级任务配置单独的模型。默认情况下，如果您的提供商有更便宜的模型可用，COdo 会尝试使用该模型，否则会回退到您的主模型。
...
您可以通过 COdo 配置中的 `theme` 选项设置要使用的主题。
...
默认代理必须是主代理（不能是子代理）。可以是内置代理（如 `"build"` 或 `"plan"`），也可以是您定义的[自定义代理](/docs/agents)。如果指定的代理不存在或是子代理，COdo 将回退到 `"build"` 并发出警告。

此设置适用于所有界面：TUI、CLI（`COdo run`）、桌面应用和 GitHub Action。
...
COdo 启动时会自动下载新版本。您可以使用 `autoupdate` 选项禁用此功能。
...
默认情况下，COdo **允许所有操作**，无需明确批准。您可以使用 `permission` 选项更改此行为。
...
[插件](/docs/plugins)通过自定义工具、钩子和集成来扩展 COdo。
...
当您希望限制 COdo 仅使用特定提供商，而不是逐一禁用其他提供商时，此选项非常有用。
