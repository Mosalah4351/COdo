---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i COdo.
---

import config from "../../../../config.mjs"
export const console = config.console

COdo bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i COdo-konfigurationen.
...
Du kan tilpasse udbyderne gennem `provider`-delen i COdo-konfigurationen.
...
COdo Zen er en liste over modeller leveret af COdo-teamet, som er blevet testet og verificeret til at fungere godt med COdo. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med COdo Zen.
:::
...
Det fungerer som alle andre udbydere i COdo og er helt valgfrit at bruge.

---

## COdo Go

COdo Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af COdo-teamet, som er testet og verificeret til at fungere godt med COdo.

1. Kør kommandoen `/connect` i TUI, vælg `COdo Go`, og gå til [codo-ai.vercel.app/auth](https://codo-ai.vercel.app/zen).
...
Sådan bruger du Amazon Bedrock med COdo:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører COdo:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `COdo.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i COdo understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere COdo til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at COdo skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem COdo-konfigurationen.
...
gemme tokenet i COdo auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
COdo bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse COdo
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`COdo.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [COdo-gitlab-auth](https://www.npmjs.com/package/COdo-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `COdo.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med COdo:
...
Sådan bruger du Google Vertex AI med COdo:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem COdo, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [COdo-helicone-session](https://github.com/H2Shami/COdo-helicone-session) plugin for automatisk at logge hver COdo-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver COdo-samtale opført som en separat session.
...
Du kan konfigurere COdo til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere COdo til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere COdo til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for COdo. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/COdo) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med COdo:
...
3. Kopier API-nøglen til brug i COdo.
...
6. **Vigtigt**: Før du bruger skymodeller i COdo, skal du hente modelinformationen lokalt:
...
COdo Zen er en liste over testede og verificerede modeller leveret af COdo-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>COdo Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **COdo Zen**.

   ```txt
   /connect
   ```

3. Indtast COdo API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem COdo-konfigurationen.
...
5. Du kan også tilpasse dem gennem COdo-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med COdo:
...
Du kan også tilpasse modeller gennem COdo-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med COdo. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `COdo.json`-filen i projektkataloget:
...
`limit`-felterne lader COdo forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `COdo auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek COdo-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i COdo-konfigurationen.
