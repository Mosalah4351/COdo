---
title: Server
description: Interager med COdo-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `COdo serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en COdo-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `COdo.local` |
...
Indstil `OPENCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `COdo`, eller indstil `OPENCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `COdo serve` og `COdo web`.
...
Når du kører `COdo` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug COdo-serveren til at interagere med COdo programmatisk.
:::

Denne arkitektur lader COdo understøtte flere klienter og lader dig interagere med COdo programmatisk.

Du kan køre `COdo serve` for at starte en selvstændig server. Hvis du har
COdo TUI kørende, vil `COdo serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af COdo [IDE](/docs/ide) plugins.
...
COdo-serveren viser følgende API'er.
