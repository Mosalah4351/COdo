...
Du kan tilføje eksterne værktøjer til COdo ved hjælp af _Model Context Protocol_ eller MCP. COdo understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [COdo Config](https://codo-ai.vercel.app/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Organisationer kan levere standard MCP-servere via deres `.well-known/COdo`-slutpunkt. Disse servere kan være deaktiveret som standard, hvilket giver brugerne mulighed for at tilmelde sig dem, de har brug for.
...
COdo håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil COdo:
...
Hvis serveren kræver godkendelse, vil COdo bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `COdo mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil COdo opbevare tokens sikkert i `~/.local/share/opencode/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem COdo-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde COdo til din Sentry-konto.
