---
title: CLI
description: COdo CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

COdo CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
COdo
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med COdo programmatisk.
...
Start COdo terminalbrugergrænsefladen.
...
COdo CLI har også følgende kommandoer.

---

### agent

Administrer agenter i COdo.
...
Tilslut en terminal til en allerede kørende COdo backend-server startet via `serve` eller `web` kommandoer.

```bash
COdo attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern COdo backend. For eksempel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `COdo`) |
...
COdo drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `opencode auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/opencode/auth.json`.

```bash
COdo auth login
```

Når COdo starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i COdo.
...
Kør COdo i ikke-interaktiv tilstand ved at sende en prompt direkte.
...
Du kan også vedhæfte til en kørende `COdo serve`-instans for at undgå MCP serverens kolde opstartstider ved hver kørsel:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Tilslut til en kørende COdo-server (f.eks. http://localhost:4096)               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Adgangskode til basic auth (standard: `OPENCODE_SERVER_PASSWORD`)                   |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `COdo`)   |
...
Start en hovedløs COdo-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.

```bash
COdo serve
```

Dette starter en HTTP-server, der giver API-adgang til COdo-funktionalitet uden TUI-grænsefladen. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `COdo`).
...
Administrator COdo sessionsholder.
...
Liste over alle COdo sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine COdo-sessioner.
...
Importer sessionsdata fra en JSON fil eller COdo del URL.

```bash
COdo import <file>
```

Du kan importere fra en lokal fil eller en COdo share URL.
...
Start en hovedløs COdo-server med en webgrænseflade.

```bash
COdo web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til COdo via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `COdo`).
...
Afinstaller COdo og fjern alle relaterede filer.
...
Opdaterer COdo til den nyeste version eller en specifik version.
...
COdo CLI tager følgende globale flag.
...
COdo kan konfigureres ved hjælp af miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `COdo`) |
