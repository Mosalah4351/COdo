---
title: Plugins
description: Skriv dine egne plugins for at udvide COdo.
---

Plugins giver dig mulighed for at udvide COdo ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre COdos standardadfærd.
...
COdo kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En COdo SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide COdo.
...
:::note
Hvis du bruger COdo desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå COdo i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til COdo:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som COdo kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for COdo sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://codo-ai.vercel.app/docs/sdk) for detaljer.
