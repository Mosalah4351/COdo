...
description: Brug COdo i enhver ACP-kompatibel editor.
---

COdo understøtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), så du kan bruge det direkte i kompatible editorer og IDE'er.
...
For at bruge COdo via ACP, konfigurer din editor til at køre kommandoen `COdo acp`.

Kommandoen starter COdo som en ACP-kompatibel underproces, der kommunikerer med din editor over JSON-RPC via stdio.
...
For at åbne den skal du bruge den nye 'COdo'-agent i AI Chat-agentvælgeren.
...
For at bruge COdo som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), skal du tilføje følgende til din Neovim-konfiguration:
...
Denne konfiguration sætter CodeCompanion til at bruge COdo som ACP-agent til chat.
...
COdo fungerer på samme måde via ACP som i terminalen. Alle funktioner understøtter:
...
- MCP-servere konfigureret i din COdo-konfiguration
