---
title: Konfiguration
description: Ved at bruge COdo JSON konfig.
---

Du kan konfigurere COdo ved hjælp af en JSON-konfigurationsfil.

---

## Format

COdo understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/COdo`) - organisatoriske standardindstillinger
...
4. **Project config** (`COdo.json` i projekt) - projektspecifikke indstillinger
...
Organisationer kan levere standardkonfiguration via `.well-known/COdo`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale COdo-konfiguration i `~/.config/opencode/opencode.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `COdo.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når COdo starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Konfigurationsfilen har et skema, der defineres i [**`codo-ai.vercel.app/config.json`**](https://codo-ai.vercel.app/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `COdo serve` og `COdo web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din COdo-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `COdo.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din COdo-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger COdo at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din COdo-konfiguration, gennem indstillingen `theme`.
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil COdo falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`COdo run`), desktop-app og GitHub Action.
...
COdo vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard COdo **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide COdo med brugerdefinerede værktøjer, kroge og integrationer.
...
Dette er nyttigt, når du vil begrænse COdo til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
