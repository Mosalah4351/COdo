---
title: Regler
description: Angiv tilpassede instruktioner til COdo.
---

Du kan gi tilpassede instruksjoner for at COdo ved at oprete en `AGENTS.md`-fil. Dette ligner på Cursors regler. Den inneholder instruksjoner som vil bli inkludert i LLMs kontekst for at tilpasse oppførselen til ditt spesifikke prosjekt.

---

## Initialisering

For at lage en ny `AGENTS.md` fil, kan du kjøre kommandoen `/init` i COdo.

:::tip
Du bør overgi prosjektets `AGENTS.md`-fil til Git.
:::

Dette vil skanne prosjektet og alt dets innhold for at forstå hva prosjektet handler om og generere en `AGENTS.md`-fil med det. Dette hjelper COdo at navigere i prosjektet bedre.
...
For brugere som migrerer fra Claude Code, støtter COdo Claude Codes filkonvensjoner som fallbacks:
...
Den anbefalte tilnærmingen er at bruge feltet `instructions` i `COdo.json`:
...
Du kan lære COdo at lese eksterne filer ved at gi eksplisitte instruksjoner i `AGENTS.md`. Her er et praktisk eksempel:
...
- Sørg for at COdo laster filer kun når det er nødvendig for den spesifikke oppgaven

:::tip
For monorepos eller prosjekter med delte standarder er brug av `COdo.json` med glob-mønstre (som `packages/*/AGENTS.md`) mer vedlikeholdbart enn manuelle instruksjoner.
:::
