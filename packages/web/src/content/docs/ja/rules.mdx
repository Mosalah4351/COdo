---
title: ルール
description: COdo のカスタム命令を設定します。
---

`AGENTS.md` ファイルを作成することで、COdo にカスタム命令を提供できます。これは Cursor のルールと似ています。これには、特定のプロジェクトに合わせて LLM の動作をカスタマイズするために LLM のコンテキストに含まれる命令が含まれています。

---

## 初期化

新しい `AGENTS.md` ファイルを作成するには、COdo で `/init` コマンドを実行します。

:::tip
プロジェクトの `AGENTS.md` ファイルを Git にコミットする必要があります。
:::
`AGENTS.md` ファイルが生成されます。これは、COdo がプロジェクトをより適切にナビゲートするのに役立ちます。
...
COdo は、複数の場所からの `AGENTS.md` ファイルの読み取りもサポートしています。そして、これはさまざまな目的に役立ちます。
...
`~/.config/opencode/AGENTS.md` ファイルにグローバルルールを含めることもできます。これは、すべての COdo セッションに適用されます。

これは Git にコミットされておらず、チームと共有されていないため、LLM が従うべき個人ルールを指定するためにこれを使用することをお勧めします。

### Claude Code 互換性

Claude Code から移行するユーザーのために、COdo はフォールバックとして Claude Code のファイル規則をサポートしています。
...
COdo が開始されると、次の順序でルールファイルが検索されます。
...
COdo は `AGENTS.md` のファイル参照を自動的に解析しませんが、次の 2 つの方法で同様の機能を実現できます。

### `COdo.json` の使用

推奨されるアプローチは、`instructions` の `COdo.json` フィールドを使用することです。
...
`AGENTS.md` で明示的な命令を指定することで、COdo に外部ファイルを読み取るように教えることができます。実際の例を次に示します。
...
- COdo が特定のタスクに必要な場合にのみファイルをロードするようにする

:::tip
モノリポジトリまたは共有標準を使用するプロジェクトの場合、グロブパターン (`COdo.json` など) で `packages/*/AGENTS.md` を使用する方が、手動で指示するよりも保守しやすくなります。
:::
