---
title: LSPサーバー
description: COdo は LSP サーバーと統合します。
---

COdo は Language Server Protocol (LSP) サーバーと統合し、診断を agent へのフィードバックとして使用できます。

---

## 組み込み

COdo には、一般的な言語用のいくつかの組み込み LSP サーバーが付属しています。
...
LSP が有効で COdo がファイルを開くと、次のようになります。
...
COdo 設定の `lsp` セクションを通じて LSP サーバーを有効化し、カスタマイズできます。
