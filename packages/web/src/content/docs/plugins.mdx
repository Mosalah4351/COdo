---
title: Plugins
description: Write your own plugins to extend COdo.
---

Plugins allow you to extend COdo by hooking into various events and customizing behavior. You can create plugins to add new features, integrate with external services, or modify COdo's default behavior.
...
COdo runs `bun install` at startup to install these. Your plugins and tools can then import them.
...
- `client`: An COdo SDK client for interacting with the AI.
...
Here are some examples of plugins you can use to extend COdo.
...
:::note
If you’re using the COdo desktop app, it can send system notifications automatically when a response is ready or when a session errors.
:::
...
Prevent COdo from reading `.env` files:
...
Plugins can also add custom tools to COdo:
...
The `tool` helper creates a custom tool that COdo can call. It takes a Zod schema function and returns a tool definition with:

- `description`: What the tool does
- `args`: Zod schema for the tool's arguments
- `execute`: Function that runs when the tool is called

Your custom tools will be available to COdo alongside built-in tools.
...
Levels: `debug`, `info`, `warn`, `error`. See [SDK documentation](https://codo-ai.vercel.app/docs/sdk) for details.
