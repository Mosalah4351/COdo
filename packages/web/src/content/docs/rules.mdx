...
You can provide custom instructions to COdo by creating an `AGENTS.md` file. This is similar to Cursor's rules. It contains instructions that will be included in the LLM's context to customize its behavior for your specific project.

---

## Initialize

To create a new `AGENTS.md` file, you can run the `/init` command in COdo.
...
COdo also supports reading the `AGENTS.md` file from multiple locations. And this serves different purposes.
...
For users migrating from Claude Code, COdo supports Claude Code's file conventions as fallbacks:
...
When COdo starts, it looks for rule files in this order:
...
While COdo doesn't automatically parse file references in `AGENTS.md`, you can achieve similar functionality in two ways:

### Using COdo.json

The recommended approach is to use the `instructions` field in `COdo.json`:
...
You can teach COdo to read external files by providing explicit instructions in your `AGENTS.md`. Here's a practical example:
...
- Ensure COdo loads files only when needed for the specific task

:::tip
For monorepos or projects with shared standards, using `COdo.json` with glob patterns (like `packages/*/AGENTS.md`) is more maintainable than manual instructions.
:::
