...
Agent skills let COdo discover reusable instructions from your repo or home directory.
...
For project-local paths, COdo walks up from your current working directory until it reaches the git worktree.
...
COdo lists available skills in the `skill` tool description.
...
Control which skills agents can access using pattern-based permissions in `COdo.json`:
...
**For built-in agents** (in `COdo.json`):
