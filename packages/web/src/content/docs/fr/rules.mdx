---
title: Règles
description: Définissez des instructions personnalisées pour COdo.
---

Vous pouvez fournir des instructions personnalisées à COdo en créant un fichier `AGENTS.md`. Ceci est similaire aux règles du curseur. Il contient des instructions qui seront incluses dans le contexte du LLM pour personnaliser son comportement pour votre projet spécifique.

---

## Initialiser

Pour créer un nouveau fichier `AGENTS.md`, vous pouvez exécuter la commande `/init` dans COdo.

:::tip
Vous devez valider le fichier `AGENTS.md` de votre projet dans Git.
:::

Cela analysera votre projet et tout son contenu pour comprendre de quoi il s'agit et générera un fichier `AGENTS.md` avec celui-ci. Cela aide COdo à mieux naviguer dans le projet.
...
COdo prend également en charge la lecture du fichier `AGENTS.md` à partir de plusieurs emplacements. Et cela répond à des objectifs différents.
...
Pour les utilisateurs migrant depuis Claude Code, COdo prend en charge les conventions de fichiers de Claude Code comme solutions de secours :
...
Lorsque COdo démarre, il recherche les fichiers de règles dans cet ordre :
...
Bien que COdo n'analyse pas automatiquement les références de fichiers dans `AGENTS.md`, vous pouvez obtenir des fonctionnalités similaires de deux manières :

### Utilisation de COdo.json

L'approche recommandée consiste à utiliser le champ `instructions` dans `COdo.json` :
...
Vous pouvez apprendre à COdo à lire des fichiers externes en fournissant des instructions explicites dans votre `AGENTS.md`. Voici un exemple pratique :
...
- Assurez-vous que COdo charge les fichiers uniquement lorsque cela est nécessaire pour une tâche spécifique.

:::tip
Pour les monorepos ou les projets avec des normes partagées, l'utilisation de `COdo.json` avec des modèles globaux (comme `packages/*/AGENTS.md`) est plus maintenable que les instructions manuelles.
:::
