---
title: Configuration
description: Utilisation de la configuration JSON COdo.
---

Vous pouvez configurer COdo à l'aide d'un fichier de configuration JSON.

---

## Format

COdo prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).
...
1. **Configuration distante** (à partir de `.well-known/COdo`) - paramètres par défaut de l'organisation
...
4. **Configuration du projet** (`COdo.json` dans le projet) - paramètres spécifiques au projet
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/COdo`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
Placez votre configuration globale COdo dans `~/.config/opencode/opencode.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.
...
Ajoutez `COdo.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Lorsque COdo démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Le fichier de configuration a un schéma défini dans [**`codo-ai.vercel.app/config.json`**](https://codo-ai.vercel.app/config.json).

La configuration TUI utilise [**`codo-ai.vercel.app/tui.json`**](https://codo-ai.vercel.app/tui.json).
...
Les anciennes clés `theme`, `keybinds` et `tui` dans `COdo.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
Vous pouvez configurer les paramètres du serveur pour les commandes `COdo serve` et `COdo web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur COdo.
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `COdo.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration COdo via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, COdo essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, COdo reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`COdo run`), application de bureau et GitHub Action.
...
COdo téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
Par défaut, COdo **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
[Plugins](/docs/plugins) étendent COdo avec des outils, des hooks et des intégrations personnalisés.
...
Ceci est utile lorsque vous souhaitez restreindre COdo à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
