...
Les compétences d'agent permettent à COdo de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
COdo recherche ces emplacements :
...
Pour les chemins locaux du projet, COdo remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
...
COdo répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `COdo.json` :
...
**Pour les agents intégrés** (dans `COdo.json`) :
