---
title: Serveur
description: Interagissez avec le serveur COdo via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `COdo serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client COdo peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `COdo.local` |
...
Définissez `OPENCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `COdo`, ou définissez `OPENCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `COdo serve` et à `COdo web`.
...
Lorsque vous exécutez `COdo`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur COdo pour interagir avec COdo par programmation.
:::

Cette architecture permet à COdo de prendre en charge plusieurs clients et vous permet d'interagir avec COdo par programme.

Vous pouvez exécuter `COdo serve` pour démarrer un serveur autonome. Si vous avez le
COdo TUI en cours d'exécution, `COdo serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins COdo [IDE](/docs/ide).
...
Le serveur COdo expose les API suivantes.
