---
title: Plugins
description: Écrivez vos propres plugins pour étendre COdo.
---

Les plugins vous permettent d'étendre COdo en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de COdo.
...
2. Configuration du projet (`COdo.json`)
...
COdo exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK COdo pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre COdo.
...
:::note
Si vous utilisez l'application de bureau COdo, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez COdo de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à COdo :
...
L'assistant `tool` crée un outil personnalisé que COdo peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour COdo aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://codo-ai.vercel.app/docs/sdk) pour plus de détails.
