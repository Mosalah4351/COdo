---
title: Support ACP
description: Utilisez COdo dans n’importe quel éditeur compatible ACP.
---

COdo prend en charge le [Agent Client Protocol](https://agentclientprotocol.com) (ACP), vous permettant de l'utiliser directement dans les éditeurs et IDE compatibles.
...
Pour utiliser COdo via ACP, configurez votre éditeur pour exécuter la commande `COdo acp`.

La commande démarre COdo en tant que sous-processus compatible ACP qui communique avec votre éditeur via JSON-RPC sur stdio.
...
```json title="acp.json"
{
  "agent_servers": {
    "COdo": {
      "command": "/absolute/path/bin/COdo",
      "args": ["acp"]
    }
  }
}
...
Pour l'ouvrir, utilisez le nouvel agent « COdo » dans le sélecteur d'agent AI Chat.
...
Pour utiliser COdo comme agent ACP dans [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), ajoutez ce qui suit à votre configuration Neovim :
...
Cette configuration configure CodeCompanion pour utiliser COdo comme agent ACP pour le chat.
...
COdo fonctionne de la même manière via ACP que dans le terminal. Toutes les fonctionnalités sont prises en charge :
...
- Serveurs MCP configurés dans votre configuration COdo
