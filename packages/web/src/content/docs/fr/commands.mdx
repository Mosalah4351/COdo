...
Vous pouvez ajouter des commandes personnalisées via la configuration COdo ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre COdo [config](/docs/config) :
...
COdo comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
