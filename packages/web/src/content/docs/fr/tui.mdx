---
title: TUI
description: Utilisation de l’interface utilisateur du terminal COdo.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

COdo fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de COdo démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le COdo TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à COdo. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Ceci est séparé de `COdo.json`, qui configure le comportement du serveur/d'exécution.
...
- `leader_timeout` - Contrôle la durée pendant laquelle COdo attend après la leader key. Valeur par défaut : `2000`.
...
- `sound_pack` - ID du sound pack à utiliser. Valeur par défaut : `COdo.default`.
