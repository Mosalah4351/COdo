---
title: ACP-støtte
description: Bruk COdo i en hvilken som helst ACP-kompatibel editor.
---

COdo støtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), slik at du kan bruke den direkte i kompatible editorer og IDE-er.
...
For å bruke COdo via ACP, konfigurer redigeringsprogrammet til å kjøre kommandoen `COdo acp`.

Kommandoen starter COdo som en ACP-kompatibel underprosess som kommuniserer med editoren din over JSON-RPC via stdio.
...
For å åpne den, bruk den nye 'COdo'-agenten i AI Chat-agentvelgeren.
...
For å bruke COdo som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), legg til følgende i Neovim-konfigurasjonen:
...
Denne konfigurasjonen setter opp CodeCompanion.nvim til å bruke COdo som ACP-agent for chat.
...
COdo fungerer på samme måte via ACP som i terminalen. Alle funksjoner støttes:
...
- MCP servere konfigurert i COdo-konfigurasjonen
