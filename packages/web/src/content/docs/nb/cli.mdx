---
title: CLI
description: COdo CLI-alternativer og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

COdo CLI starter som standard [TUI](/docs/tui) når den kjøres uten noen argumenter.

```bash
COdo
```

Men den godtar også kommandoer som dokumentert på denne siden. Dette lar deg samhandle med COdo programmatisk.
...
Start COdo-terminalbrukergrensesnittet.
...
Koble en terminal til en allerede kjørende COdo-backend-server startet via `serve` eller `web` kommandoer.

```bash
COdo attach [url]
```

Dette gjør det mulig å bruke TUI med en ekstern COdo-backend. For eksempel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `COdo`) |
...
COdo drives av leverandørlisten på [Models.dev](https://models.dev), så du kan bruke `opencode auth login` til å konfigurere API-nøkler for enhver leverandør du vil bruke. Dette er lagret i `~/.local/share/opencode/auth.json`.

```bash
COdo auth login
```

Når COdo starter opp, laster den leverandørene fra filen med påloggingsinformasjon. Og hvis det er noen nøkler definert i miljøene dine eller en `.env`-fil i prosjektet ditt.
...
Bruk `--refresh`-flagget for å oppdatere den bufrede modelllisten. Dette er nyttig når nye modeller er lagt til en leverandør og du vil se dem i COdo.
...
Kjør COdo i ikke-interaktiv modus ved å sende en melding direkte.
...
Du kan også koble til en kjørende `COdo serve`-forekomst for å unngå MCP-serveres kaldstarttid ved hver kjøring:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Koble til en kjørende COdo-server (f.eks. http://localhost:4096)                               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Passord for grunnleggende autentisering (standard: `OPENCODE_SERVER_PASSWORD`)                     |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `COdo`) |
...
Start en headless COdo-server for API-tilgang. Sjekk ut [server-dokumentene](/docs/server) for hele HTTP-grensesnittet.

```bash
COdo serve
```

Dette starter en HTTP-server som gir API tilgang til COdo-funksjonalitet uten TUI-grensesnittet. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `COdo`).
...
Vis tokenbruk og kostnadsstatistikk for COdo-øktene dine.
...
Importer øktdata fra en JSON-fil eller en COdo share-URL.
...
Start en headless COdo-server med et webgrensesnitt.

```bash
COdo web
```

Dette starter en HTTP-server og åpner en nettleser for å få tilgang til COdo via et nettgrensesnitt. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `COdo`).
...
Avinstaller COdo og fjern alle relaterte filer.
...
Oppdaterer COdo til siste versjon eller en spesifikk versjon.
...
COdo CLI bruker følgende globale flagg.
...
COdo kan konfigureres ved hjelp av miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | streng | Overstyr grunnleggende autentiseringsbrukernavn (standard `COdo`) |
