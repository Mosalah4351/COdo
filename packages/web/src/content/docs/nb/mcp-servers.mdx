...
Du kan legge til eksterne verktøy i COdo ved å bruke _Model Context Protocol_ eller MCP. COdo støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [COdo Config](https://codo-ai.vercel.app/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="COdo.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/COdo`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
COdo håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil COdo:
...
Hvis serveren krever autentisering, vil COdo be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `COdo mcp auth <server-name>`.
...
```bash
COdo mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil COdo lagre tokens sikkert i `~/.local/share/opencode/mcp-auth.json`.
...
COdo mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i COdo, sammen med innebygde verktøy. Så du kan administrere dem gjennom COdo-konfigurasjonen som alle andre verktøy.
...
```bash
COdo mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble COdo til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver COdo-samtale oppført som en separat økt.
