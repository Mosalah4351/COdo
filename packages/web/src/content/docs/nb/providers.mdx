---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i COdo.
---

import config from "../../../../config.mjs"
export const console = config.console

COdo bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i COdo-konfigurasjonen.
...
Du kan tilpasse leverandørene gjennom `provider`-delen i COdo
konfig.
...
COdo Zen er en liste over modeller levert av COdo-teamet som har vært
testet og verifisert for å fungere godt med COdo. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med COdo Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `COdo Zen` og gå til [codo-ai.vercel.app/auth](https://codo-ai.vercel.app/zen).
...
Det fungerer som alle andre leverandører i COdo og er helt valgfritt å bruke.

---

## COdo Go

COdo Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av COdo-teamet som har vært
testet og verifisert for å fungere godt med COdo.

1. Kjør kommandoen `/connect` i TUI, velg `COdo Go`, og gå til [codo-ai.vercel.app/auth](https://codo-ai.vercel.app/zen).
...
Slik bruker du Amazon Bedrock med COdo:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører COdo:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `COdo.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i COdo støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere COdo til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at COdo skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom COdo-konfigurasjonen.
...
for å lagre token i COdo auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
COdo bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse COdo
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`COdo.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [COdo-gitlab-auth](https://www.npmjs.com/package/COdo-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `COdo.json`:
...
Slik bruker du GitHub Copilot-abonnementet med COdo:
...
Slik bruker du Google Vertex AI med COdo:
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom COdo, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [COdo-helicone-session](https://github.com/H2Shami/COdo-helicone-session) plugin for automatisk å logge hver COdo-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver COdo-samtale oppført som en separat økt.
...
Du kan konfigurere COdo for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere COdo for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere COdo for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for COdo. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/COdo) for detaljer.
:::
...
Slik bruker du Ollama Cloud med COdo:
...
3. Kopier API-nøkkelen for bruk i COdo.
...
6. **Viktig**: Før du bruker skymodeller i COdo, må du hente modellinformasjonen lokalt:
...
COdo Zen er en liste over testede og verifiserte modeller levert av COdo-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>COdo Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **COdo Zen**.

   ```txt
   /connect
   ```

3. Skriv inn COdo API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom COdo-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom COdo-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med COdo:
...
Du kan også tilpasse modeller gjennom COdo-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med COdo. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `COdo.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar COdo forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `COdo auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk COdo-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i COdo-konfigurasjonen.
