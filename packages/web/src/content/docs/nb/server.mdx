---
title: Tjener
description: Samhandle med COdo-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `COdo serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en COdo-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `COdo.local` |
...
Sett `OPENCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `COdo` som standard, eller sett `OPENCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `COdo serve` og `COdo web`.
...
Når du kjører `COdo` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk COdo-serveren til å samhandle med COdo programmatisk.
:::

Denne arkitekturen lar COdo støtte flere klienter og lar deg samhandle med COdo programmatisk.

Du kan kjøre `COdo serve` for å starte en frittstående server. Hvis du har
COdo TUI kjører, vil `COdo serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av COdo [IDE](/docs/ide) plugins.
...
COdo-serveren viser følgende APIer.
