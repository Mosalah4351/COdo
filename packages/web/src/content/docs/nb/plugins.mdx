...
description: Skriv dine egne programtillegg for å utvide COdo.
---

Plugins lar deg utvide COdo ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til COdo.
...
2. Prosjektkonfigurasjon (`COdo.json`)
...
COdo kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En COdo SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide COdo.
...
:::note
Hvis du bruker COdo-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre COdo fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til COdo:
...
`tool`-hjelperen lager et tilpasset verktøy som COdo kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for COdo sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://codo-ai.vercel.app/docs/sdk) for detaljer.
