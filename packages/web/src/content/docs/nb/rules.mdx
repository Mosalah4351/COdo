---
title: Regler
description: Set custom instructions for COdo.
---

Du kan gi tilpassede instruksjoner for å COdo ved å opprette en `AGENTS.md`-fil. Dette ligner på Cursors regler. Den inneholder instruksjoner som vil bli inkludert i LLMs kontekst for å tilpasse oppførselen til ditt spesifikke prosjekt.

---

## Initialisering

For å lage en ny `AGENTS.md` fil, kan du kjøre kommandoen `/init` i COdo.

:::tip
Du bør sjekke inn prosjektets `AGENTS.md`-fil i Git.
:::

Dette vil skanne prosjektet og alt dets innhold for å forstå hva prosjektet handler om og generere en `AGENTS.md`-fil med det. Dette hjelper COdo å navigere i prosjektet bedre.
...
COdo støtter også lesing av `AGENTS.md`-filen fra flere steder. Og dette tjener forskjellige formål.
...
Du kan også ha globale regler i en `~/.config/opencode/AGENTS.md`-fil. Dette blir brukt på alle COdo-økter.
...
For brukere som migrerer fra Claude Code, støtter COdo Claude Codes filkonvensjoner som fallbacks:
...
Når COdo starter, ser den etter regelfiler i denne rekkefølgen:
...
Selv om COdo ikke automatisk analyserer filreferanser i `AGENTS.md`, kan du oppnå lignende funksjonalitet på to måter:

### Bruker COdo.json

Den anbefalte tilnærmingen er å bruke feltet `instructions` i `COdo.json`:
...
Du kan lære COdo å lese eksterne filer ved å gi eksplisitte instruksjoner i `AGENTS.md`. Her er et praktisk eksempel:
...
- Sørge for at COdo laster filer kun når det er nødvendig for den spesifikke oppgaven

:::tip
For monorepos eller prosjekter med delte standarder er bruk av `COdo.json` med glob-mønstre (som `packages/*/AGENTS.md`) mer vedlikeholdbart enn manuelle instruksjoner.
:::
