...
For å feilsøke problemer med COdo, start med å sjekke loggene og lokale data den lagrer på disken.
...
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.local\share\COdo\log`

Loggfiler navngis med tidsstempler (f.eks. `2025-01-09T123456.log`) og de siste 10 loggfilene beholdes.

Du kan angi loggnivået med kommandolinjealternativet `--log-level` for å få mer detaljert feilsøkingsinformasjon. For eksempel `COdo --log-level DEBUG`.

---

## Lagring

COdo lagrer øktdata og andre applikasjonsdata på disken på:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.local\share\COdo`
...
COdo Desktop kjører en lokal COdo-server (`COdo-cli` sidevognen) i bakgrunnen. De fleste problemene er forårsaket av en plugin som fungerer dårlig, en ødelagt cache eller en dårlig serverinnstilling.

### Raske sjekker

- Avslutt og start appen på nytt.
- Hvis appen viser en feilskjerm, klikker du på **Start på nytt** og kopierer feildetaljene.
- Bare macOS: `COdo`-meny -> **Last nettvisning på nytt** (hjelper hvis UI er tom/frosset).
...
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.config\COdo\COdo.jsonc`
...
COdo kan også laste lokale plugins fra disken. Flytt disse midlertidig ut av veien (eller gi nytt navn til mappen) og start skrivebordsappen på nytt:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.config\COdo\plugins`
...
Hvis deaktivering av plugins ikke hjelper (eller en plugin-installasjon sitter fast), tøm hurtigbufferen slik at COdo kan gjenoppbygge den.

1. Avslutt COdo Desktop helt.
2. Slett hurtigbufferkatalogen:

- **macOS**: Finder -> `Cmd+Shift+G` -> lim inn `~/.cache/COdo`
- **Linux**: slett `~/.cache/COdo` (eller kjør `rm -rf ~/.cache/COdo`)
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.cache\COdo`

3. Start COdo Desktop på nytt.

---

### Løs problemer med servertilkobling

COdo Desktop kan enten starte sin egen lokale server (standard) eller koble til en server URL du har konfigurert.
...
Hvis `COdo.json(c)` inneholder en `server`-del, fjern den midlertidig og start skrivebordsappen på nytt.
...
På Windows krever COdo Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åpnes i et tomt vindu eller ikke starter, installer/oppdater WebView2 og prøv igjen.

---

### Windows: Generelle ytelsesproblemer

Hvis du opplever treg ytelse, problemer med filtilgang eller terminalproblemer på Windows, kan du prøve å bruke [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL gir et Linux-miljø som fungerer mer sømløst med funksjonene til COdo.

---

### Varsler vises ikke

COdo Desktop viser bare systemvarsler når:

- varsler er aktivert for COdo i OS-innstillingene dine, og
...
1. Avslutt COdo Desktop.
2. Finn og slett disse filene (de finnes i COdo Desktop-appens datakatalog):

- `COdo.settings.dat` (stasjonær standardserver URL)
- `COdo.global.dat` og `COdo.workspace.*.dat` (UI tilstand som nylige servere/prosjekter)
...
Hvis du støter på API-anropsfeil, kan dette skyldes utdaterte provider-pakker. COdo installerer dynamisk provider-pakker (OpenAI, Anthropic, Google, etc.) etter behov og cacher dem lokalt.
...
1. Tøm leverandørens pakkebuffer:
   ...
   På Windows, trykk `WIN+R` og slett: `%USERPROFILE%\.cache\COdo`

2. Start COdo på nytt for å installere de nyeste provider-pakkene på nytt

Dette vil tvinge COdo til å laste ned de nyeste versjonene av provider-pakkene, som ofte løser kompatibilitetsproblemer med modellparametere og API-endringer.
...
COdo vil oppdage om du bruker Wayland og foretrekker `wl-clipboard`, ellers vil den prøve å finne utklippstavle-verktøy i rekkefølgen: `xclip` og `xsel`.
