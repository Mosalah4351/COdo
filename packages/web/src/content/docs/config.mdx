---
title: Config
description: Using the COdo JSON config.
---

You can configure COdo using a JSON config file.

---

## Format

COdo supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="COdo.jsonc"
{
...
1. **Remote config** (from `.well-known/COdo`) - organizational defaults
2. **Global config** (`~/.config/opencode/opencode.json`) - user preferences
3. **Custom config** (`OPENCODE_CONFIG` env var) - custom overrides
4. **Project config** (`COdo.json` in project) - project-specific settings
...
Organizations can provide default configuration via the `.well-known/COdo` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/COdo"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global COdo config in `~/.config/opencode/opencode.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.
...
Add `COdo.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When COdo starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Drop an `COdo.json` or `COdo.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/opencode/` |
| Linux    | `/etc/opencode/`                         |
| Windows  | `%ProgramData%\COdo`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, COdo reads managed preferences from the `ai.opencode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

COdo checks these paths:

1. `/Library/Managed Preferences/<user>/ai.opencode.managed.plist`
2. `/Library/Managed Preferences/ai.opencode.managed.plist`

The plist keys map directly to `COdo.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.opencode.managed` PayloadType. The COdo config keys go directly in the payload dict:
...
The server/runtime config schema is defined in [**`codo-ai.vercel.app/config.json`**](https://codo-ai.vercel.app/config.json).

TUI config uses [**`codo-ai.vercel.app/tui.json`**](https://codo-ai.vercel.app/tui.json).
...
```json title="tui.json"
{
  "$schema": "https://codo-ai.vercel.app/tui.json",
  "scroll_speed": 3,
  "scroll_acceleration": {
    "enabled": true
  },
...
Legacy `theme`, `keybinds`, and `tui` keys in `COdo.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `COdo serve` and `COdo web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your COdo server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `COdo.local`. Useful for running multiple instances on the same network.
...
If not specified, COdo will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your COdo config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, COdo tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny COdo actions on configured resources. Currently, policies can control which providers COdo may use.
...
COdo normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, COdo omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, COdo will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`COdo run`), desktop app, and GitHub Action.
...
COdo uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
COdo will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, COdo **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend COdo with custom tools, hooks, and integrations.
...
This is useful when you want to restrict COdo to only use specific providers rather than disabling them one by one.
