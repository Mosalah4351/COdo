---
title: TUI
description: Using the COdo terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

COdo provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running COdo starts the TUI for the current directory.
...
When using the COdo TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to COdo. Allows you to select from available providers and add their API keys.
...
This is separate from `COdo.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long COdo waits after the leader key. Defaults to `2000`.
...
- `sound_pack` - Sound pack ID to use. Defaults to `COdo.default`.
