...
Možete dodati vanjske alate u COdo koristeći _Model Context Protocol_, ili MCP. COdo podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [COdo Config](https://codo-ai.vercel.app/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/COdo`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
COdo automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, COdo će:
...
Ako server zahtijeva autentifikaciju, COdo će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `COdo mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, COdo sigurno čuva tokene u `~/.local/share/opencode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u COdo, zajedno s ugrađenim alatima. Možete njima upravljati kroz COdo konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete COdo sa vašim Sentry nalogom.
