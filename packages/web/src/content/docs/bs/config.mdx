---
title: Konfiguracija
description: Korištenje COdo JSON konfiguracije.
---

Možete konfigurirati COdo koristeći JSON konfiguracijski fajl.

---

## Format

COdo podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/COdo`) - organizacijske postavke
...
4. **Konfiguracija projekta** (`COdo.json` u projektu) - postavke specifične za projekat
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/COdo` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu COdo konfiguraciju u `~/.config/opencode/opencode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.
...
Dodajte `COdo.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se COdo pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Konfiguracijski fajl ima šemu koja je definirana u [**`codo-ai.vercel.app/config.json`**](https://codo-ai.vercel.app/config.json).

TUI konfiguracija koristi [**`codo-ai.vercel.app/tui.json`**](https://codo-ai.vercel.app/tui.json).
...
Stari `theme`, `keybinds`, i `tui` ključevi u `COdo.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `COdo serve` i `COdo web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš COdo server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `COdo.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj COdo konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, COdo pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, COdo će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`COdo run`), desktop aplikaciju i GitHub Action.
...
COdo će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, COdo **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju COdo sa prilagođenim alatima, kukicama i integracijama.
...
Ovo je korisno kada želite da ograničite COdo da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
