---
title: Dodaci
description: Napišite vlastite dodatke za proširenje COdo.
---

Dodaci vam omogućavaju da proširite COdo spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja COdo.
...
2. Konfiguracija projekta (`COdo.json`)
...
COdo pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje COdo.
...
Spriječite COdo da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u COdo:
...
Pomoćnik `tool` kreira prilagođeni alat koji COdo može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za COdo zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://codo-ai.vercel.app/docs/sdk) za detalje.
