...
COdo Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u COdo-u. Pretplatite se na COdo Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili COdo.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa COdo-om.
:::
...
COdo Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

COdo Go radi kao bilo koji drugi provajder u COdo-u.

1. Prijavite se na **<a href={console}>COdo Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `COdo Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na COdo Go.
:::
...
COdo Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj COdo konfiguraciji
koristi format `COdo-go/<model-id>`. Na primjer, za Kimi K2.7 Code, koristili biste
`COdo-go/kimi-k2.7-code` u svojoj konfiguraciji.
...
Napravili smo COdo Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz COdo koristite i bilo kojeg drugog provajdera.
