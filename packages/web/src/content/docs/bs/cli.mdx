---
title: CLI
description: COdo CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

COdo CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
COdo
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa COdo.
...
Pokrenite COdo terminalski korisnički interfejs.
...
COdo CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte COdo agentima.
...
Priključite terminal na već pokrenut COdo backend server pokrenut putem `serve` ili `web` naredbi.

```bash
COdo attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim COdo backend-om. Na primjer:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `COdo`) |
...
COdo pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `opencode auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/opencode/auth.json`.

```bash
COdo auth login
```

Kada se COdo pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u COdo.
...
Pokrenite COdo u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
Također možete priključiti pokrenutu `COdo serve` instancu kako biste izbjegli vrijeme hladnog pokretanja MCP servera pri svakom pokretanju:
...
| <nobr><code>{"--attach"}</code></nobr>   |        | Priključite na pokrenuti COdo server (npr. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p`   | Lozinka za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_PASSWORD`)                       |
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `COdo`) |
...
Pokrenite COdo headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
COdo serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti COdo-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `COdo`).
...
Prikaži statistiku upotrebe tokena i troškova za vaše COdo sesije.
...
Uvezite podatke sesije iz JSON datoteke ili COdo dijeljenog URL-a.

```bash
COdo import <file>
```

Možete uvesti iz lokalne datoteke ili COdo dijeljenog URL-a.
...
Pokrenite COdo headless server sa web interfejsom.

```bash
COdo web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup COdo-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `COdo`).
...
Deinstalirajte COdo i uklonite sve povezane datoteke.
...
Ažurira COdo na najnoviju verziju ili određenu verziju.
...
COdo CLI prihvata sljedeće globalne zastavice.
...
COdo se može konfigurirati pomoću varijabli okruženja.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Poništi osnovno korisničko ime autentifikacije (zadano `COdo`) |
