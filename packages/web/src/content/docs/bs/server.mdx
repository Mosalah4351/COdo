---
title: Server
description: Komunicirajte s COdo serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `COdo serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji COdo klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `COdo.local` |
...
Postavite `OPENCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `COdo`, ili postavite `OPENCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `COdo serve` i za `COdo web`.
...
Kada pokrenete `COdo`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite COdo server za programsku interakciju sa COdo-om.
:::

Ova arhitektura omogucava COdo podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `COdo serve` da startate standalone server. Ako je COdo TUI vec pokrenut, `COdo serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste COdo [IDE](/docs/ide) pluginovi.
...
COdo server izlaže sljedece API-je.
