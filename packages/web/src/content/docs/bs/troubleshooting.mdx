...
Da biste otklonili probleme s COdo, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\COdo\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `COdo --log-level DEBUG`.

---

## Pohrana

COdo pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\COdo`
...
COdo Desktop pokreće lokalni COdo server (`COdo-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `COdo` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\COdo\COdo.jsonc`
...
COdo također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\COdo\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga COdo mogao ponovo izgraditi.

1. Potpuno zatvorite COdo Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/COdo`
- **Linux**: obrišite `~/.cache/COdo` (ili pokrenite `rm -rf ~/.cache/COdo`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\COdo`

3. Ponovo pokrenite COdo Desktop.

---

### Rješavanje problema sa vezom na serveru

COdo Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `COdo.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.
...
Na Windows-u, COdo Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa COdo karakteristikama.

---

### Obavještenja se ne prikazuju

COdo Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za COdo u postavkama vašeg OS-a, i
...
1. Zatvorite COdo Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka COdo Desktop aplikacije):

- `COdo.settings.dat` (URL zadanog servera za desktop)
- `COdo.global.dat` i `COdo.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
3. Uvjerite se da imate najnoviju verziju sa `COdo upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `COdo models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\COdo`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. COdo dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\COdo`

2. Ponovo pokrenite COdo da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti COdo da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
COdo će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
