---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u COdo.
---

import config from "../../../../config.mjs"
export const console = config.console

COdo koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj COdo konfiguraciji.
...
Možete prilagoditi dobavljače putem odjeljka `provider` u vašem COdo
config.
...
COdo Zen je lista modela koje je obezbedio COdo tim koji su bili
testirano i potvrđeno da dobro radi sa COdo. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa COdo Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite COdo i idite na [codo-ai.vercel.app/auth](https://codo-ai.vercel.app/auth).
...
Radi kao i svaki drugi provajder u COdo i potpuno je opcionalan za korištenje.

---

## COdo Go

COdo Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža COdo tim i koji su testirani i verificirani da dobro rade s COdo-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `COdo Go` i idite na [codo-ai.vercel.app/auth](https://codo-ai.vercel.app/zen).
...
Da biste koristili Amazon Bedrock s COdo:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete COdo:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `COdo.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u COdo.
:::
...
Možete konfigurirati COdo za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi COdo ispravno radio.
   :::
...
Također možete dodati modele kroz svoju COdo konfiguraciju.
...
da pohrani token u COdo auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
COdo koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate COdo
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`COdo.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [COdo-gitlab-auth](https://www.npmjs.com/package/COdo-gitlab-auth).

##### Konfiguracija

Prilagodite putem `COdo.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s COdo:
...
Za korištenje Google Vertex AI s COdo:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće COdo.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [COdo-helicone-session](https://github.com/H2Shami/COdo-helicone-session) da automatski prijavite svaki COdo razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki COdo razgovor naveden kao zasebna sesija.
...
Možete konfigurirati COdo za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati COdo za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati COdo za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za COdo. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/COdo) za detalje.
:::
...
Da biste koristili Ollama Cloud s COdo:
...
3. Kopirajte API ključ za korištenje u COdo.
...
6. **Važno**: Prije upotrebe modela oblaka u COdo, morate lokalno povući informacije o modelu:
...
COdo Zen je lista testiranih i verifikovanih modela koju je obezbedio COdo tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>COdo Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **COdo Zen**.

```txt
   /connect
```

3. Unesite svoj COdo API ključ.
...
Također možete dodati dodatne modele putem vaše COdo konfiguracije.
...
5. Također ih možete prilagoditi putem vaše COdo konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju COdo konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s COdo-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `COdo.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju COdo da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `COdo auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite COdo konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj COdo konfiguraciji.
