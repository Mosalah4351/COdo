...
Agent skills omogucavaju COdo da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Za projektne lokalne putanje, COdo ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
...
COdo navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `COdo.json`:
...
**Za ugradene agente** (u `COdo.json`):
