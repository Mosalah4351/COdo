---
title: Pravila
description: Postavite prilagodena uputstva za COdo.
---

Mozete dodati prilagodena uputstva za COdo tako sto kreirate `AGENTS.md` datoteku. Ovo je slicno pravilima u Cursor-u. Sadrzi uputstva koja se ubacuju u LLM kontekst da prilagode ponasanje za vas projekat.

---

## Inicijalizacija

Da kreirate novu `AGENTS.md` datoteku, pokrenite `/init` komandu u COdo.

:::tip
Preporuceno je da `AGENTS.md` iz projekta commitujete u Git.
:::

Ovo skenira projekat i njegov sadrzaj, razumije cemu projekat sluzi i generise `AGENTS.md`. Tako COdo bolje navigira kroz kod.
...
COdo podrzava citanje `AGENTS.md` datoteke sa vise lokacija. Svaka lokacija ima drugu svrhu.
...
Za korisnike koji prelaze sa Claude Code, COdo podrzava i Claude konvencije datoteka kao rezervu:
...
Kada se COdo pokrene, trazi datoteke pravila ovim redoslijedom:
...
Iako COdo ne parsira automatski reference datoteka u `AGENTS.md`, slicno ponasanje mozete dobiti na dva nacina:

### Korištenje COdo.json

Preporuceni pristup je da koristite `instructions` polje u `COdo.json`:
...
Mozete nauciti COdo da cita eksterne datoteke tako sto cete dati eksplicitna uputstva u `AGENTS.md`. Evo prakticnog primjera:
...
- Osigurate da COdo ucitava datoteke samo kad su potrebne za konkretan zadatak

:::tip
Za monorepo projekte ili projekte sa zajednickim standardima, odrzivije je koristiti `COdo.json` sa glob obrascima (npr. `packages/*/AGENTS.md`) nego rucna uputstva.
:::
