---
title: Complementos
description: Escriba sus propios complementos para extender COdo.
---

Los complementos le permiten extender COdo al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de COdo.
...
2. Configuración del proyecto (`COdo.json`)
...
COdo ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK COdo para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar COdo.
  ...
  return {
    event: async ({ event }) => {
      ...
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "COdo"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio COdo, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que COdo lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a COdo:

```ts title=".opencode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@COdo-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que COdo puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para COdo junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://codo-ai.vercel.app/docs/sdk) para obtener más detalles.
