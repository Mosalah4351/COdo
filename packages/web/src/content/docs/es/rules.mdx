---
title: Reglas
description: Establezca instrucciones personalizadas para COdo.
---

Puede proporcionar instrucciones personalizadas a COdo creando un archivo `AGENTS.md`. Esto es similar a las reglas del Cursor. Contiene instrucciones que se incluirán en el contexto de LLM para personalizar su comportamiento para su proyecto específico.

---

## Inicializar

Para crear un nuevo archivo `AGENTS.md`, puede ejecutar el comando `/init` en COdo.

:::tip
Debes enviar el archivo `AGENTS.md` de tu proyecto a Git.
:::

Esto escaneará su proyecto y todo su contenido para comprender de qué se trata y generar un archivo `AGENTS.md` con él. Esto ayuda a COdo a navegar mejor por el proyecto.
...
COdo también admite la lectura del archivo `AGENTS.md` desde múltiples ubicaciones. Y esto tiene diferentes propósitos.
...
Para los usuarios que migran desde Claude Code, COdo admite las convenciones de archivos de Claude Code como alternativas:
...
Cuando se inicia COdo, busca archivos de reglas en este orden:
...
Si bien COdo no analiza automáticamente las referencias de archivos en `AGENTS.md`, puedes lograr una funcionalidad similar de dos maneras:

### Usar COdo.json

El enfoque recomendado es utilizar el campo `instructions` en `COdo.json`:
...
Puede enseñarle a COdo a leer archivos externos proporcionando instrucciones explícitas en su `AGENTS.md`. He aquí un ejemplo práctico:
...
- Asegúrese de que COdo cargue archivos solo cuando sea necesario para la tarea específica

:::tip
Para monorepos o proyectos con estándares compartidos, usar `COdo.json` con patrones globales (como `packages/*/AGENTS.md`) es más fácil de mantener que las instrucciones manuales.
:::
