...
Las habilidades del agente permiten a COdo descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Para las rutas locales del proyecto, COdo sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
...
COdo enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `COdo.json`:
...
**Para agentes integrados** (en `COdo.json`):
