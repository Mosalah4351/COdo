---
title: CLI
description: Opciones y comandos de la CLI de COdo.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de COdo de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
COdo
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con COdo mediante programación.
...
Inicie la interfaz de usuario del terminal COdo.
...
La CLI de COdo también tiene los siguientes comandos.

---

### agent

Administrar agentes para COdo.
...
Conecte una terminal a un servidor backend COdo que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
COdo attach [url]
```

Esto permite usar TUI con un backend remoto COdo. Por ejemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `COdo`) |
...
COdo funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `opencode auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/opencode/auth.json`.

```bash
COdo auth login
```

Cuando se inicia COdo, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en COdo.
...
Ejecute COdo en modo no interactivo pasando un mensaje directamente.
...
También puede conectarse a una instancia `COdo serve` en ejecución para evitar tiempos de arranque en frío del servidor MCP en cada ejecución:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Adjuntar a un servidor COdo en ejecución (por ejemplo, http://localhost:4096)         |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Contraseña de autenticación básica (predeterminada: `OPENCODE_SERVER_PASSWORD`)           |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `COdo`) |
...
Inicie un servidor COdo sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.

```bash
COdo serve
```

Esto inicia un servidor HTTP que proporciona acceso API a la funcionalidad COdo sin la interfaz TUI. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `COdo`).
...
Enumere todas las sesiones COdo.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones COdo.
...
Importe datos de sesión desde un archivo JSON o una URL compartida COdo.

```bash
COdo import <file>
```

Puede importar desde un archivo local o una URL compartida COdo.
...
Inicie un servidor COdo sin cabeza con una interfaz web.

```bash
COdo web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a COdo a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `COdo`).
...
Desinstale COdo y elimine todos los archivos relacionados.
...
Actualiza COdo a la última versión o a una versión específica.
...
La CLI de COdo toma las siguientes banderas globales.
...
COdo se puede configurar mediante variables de entorno.
...
| `OPENCODE_SERVER_USERNAME`            | cadena   | Anular el nombre de usuario de autenticación básica (predeterminado `COdo`) |
