---
title: Servidor
description: Interactuar con el servidor COdo a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `COdo serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente COdo puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `COdo.local` |
...
Configure `OPENCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `COdo`, o configure `OPENCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `COdo serve` como a `COdo web`.
...
Cuando ejecuta `COdo`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor COdo para interactuar con COdo mediante programación.
:::

Esta arquitectura permite que COdo admita múltiples clientes y le permite interactuar con COdo mediante programación.

Puede ejecutar `COdo serve` para iniciar un servidor independiente. Si tienes el
COdo TUI ejecutándose, `COdo serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos COdo [IDE](/docs/ide).
...
El servidor COdo expone las siguientes API.
