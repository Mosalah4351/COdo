...
Para depurar problemas con COdo, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\COdo\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `COdo --log-level DEBUG`.

---

## Almacenamiento

COdo almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\COdo`
...
COdo Desktop ejecuta un servidor COdo local (el sidecar `COdo-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `COdo` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\COdo\COdo.jsonc`
...
COdo también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\COdo\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que COdo pueda reconstruirlo.

1. Salga de COdo Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/COdo`
- **Linux**: eliminar `~/.cache/COdo` (o ejecutar `rm -rf ~/.cache/COdo`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\COdo`

3. Reinicie el escritorio COdo.

---

### Solucionar problemas de conexión del servidor

COdo Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `COdo.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.
...
En Windows, el escritorio COdo requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de COdo.

---

### Notificaciones que no se muestran

COdo Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para COdo en la configuración de su sistema operativo, y
...
1. Salga del escritorio COdo.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio COdo):

- `COdo.settings.dat` (URL del servidor predeterminado de escritorio)
- `COdo.global.dat` y `COdo.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
3. Asegúrese de tener la última versión con `COdo upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `COdo models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\COdo`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. COdo instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\COdo`

2. Reinicie COdo para reinstalar los paquetes del proveedor más recientes.

Esto obligará a COdo a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
COdo detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
