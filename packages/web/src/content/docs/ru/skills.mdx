...
Навыки агента позволяют COdo обнаруживать многократно используемые инструкции из вашего репозитория или домашнего каталога.
...
COdo выполняет поиск в следующих местах:
...
Для локальных путей проекта COdo переходит из вашего текущего рабочего каталога, пока не достигнет рабочего дерева git.
...
COdo перечисляет доступные навыки в описании инструмента `skill`.
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `COdo.json`:
...
**Для встроенных агентов** (в формате `COdo.json`):
