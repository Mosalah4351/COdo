...
import { createCOdo, type ToolPart } from "@codo-ai/sdk"
...
console.log("🚀 Starting COdo server...")
const COdo = await createCOdo({
  port: 0,
})
console.log("✅ COdo server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await COdo.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new COdo session...")
    const { client, server } = COdo
    ...
    console.log("✅ Created COdo session:", createResult.data.id)
  ...
  console.log("📝 Sending to COdo:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 COdo response:", JSON.stringify(result, null, 2))
