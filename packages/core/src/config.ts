...
export class Service extends Context.Service<Service, Interface>()("@codo/v2/Config") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = ["config.json", "codo.json", "codo.jsonc"]
    ...
    const discovered = locationIsGlobal
      ...
      : yield* fs
          .up({
            targets: [".COdo", ".opencode", ...names.toReversed()],
    ...
    const directPaths = discovered.filter((item) => path.basename(item) !== ".COdo" && path.basename(item) !== ".opencode").toReversed()
    ...
    // global config, project files, then `.COdo` files.
