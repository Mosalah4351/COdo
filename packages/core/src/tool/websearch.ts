...
import { ToolFailure } from "@codo-ai/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@codo/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.CODO_WEBSEARCH_PROVIDER === "exa" || process.env.CODO_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.CODO_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("CODO_EXPERIMENTAL") || truthy("CODO_ENABLE_EXA") || truthy("CODO_EXPERIMENTAL_EXA"),
    enableParallel: truthy("CODO_ENABLE_PARALLEL") || truthy("CODO_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `COdo/${InstallationVersion}`,
