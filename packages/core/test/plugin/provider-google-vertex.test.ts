...
import { GoogleVertexPlugin } from "@codo-ai/core/plugin/provider/google-vertex"
  ...
  it.effect("ignores OpenAI-compatible providers that are not Google Vertex", () =>
    Effect.gen(function* () {
      ...
      yield* transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.COdo, (provider) => {
          provider.api = {
            type: "aisdk",
            package: "@ai-sdk/openai-compatible",
            url: "https://codo-ai.vercel.app/zen/v1",
          }
        }),
      )

      const provider = yield* catalog.provider.get(ProviderV2.ID.COdo)
