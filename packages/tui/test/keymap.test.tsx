...
import { getCOdoModeStack, CODO_BASE_MODE, COdoKeymapProvider, registerCOdoKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerCOdoKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when opencode mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getCOdoModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getCOdoModeStack(keymap).push("autocomplete")
