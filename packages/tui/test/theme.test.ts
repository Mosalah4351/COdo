...
test("addTheme writes into module theme store", () => {
  ...
  expect(addTheme(name, DEFAULT_THEMES.cobalt2)).toBe(true)
...
test("addTheme keeps first theme for duplicate names", () => {
  ...
  const one = structuredClone(DEFAULT_THEMES.cobalt2)
  const two = structuredClone(DEFAULT_THEMES.cobalt2)
...
test("resolveTheme rejects circular color refs", () => {
  const item = structuredClone(DEFAULT_THEMES.cobalt2)
