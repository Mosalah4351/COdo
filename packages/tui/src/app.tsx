  ...
  const result = yield* Effect.scoped(
    Effect.gen(function* () {
      ...
      yield* Effect.acquireRelease(
        Effect.sync(() => registerCOdoKeymap(keymap, renderer, input.config)),
                ...
                <ErrorBoundary fallback={(error, reset) => <ErrorComponent error={error} reset={reset} mode={mode} />}>
                  ...
                  >
                    ...
                    >
                      <TuiStartupProvider
                        value={{
                          initialRoute: process.env.CODO_ROUTE ? JSON.parse(process.env.CODO_ROUTE) : undefined,
                          skipInitialLoading: Boolean(process.env.CODO_FAST_BOOT),
                        }}
                      >
                        <ClipboardProvider>
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to COdo v${result.data.version}. Please restart the application.`,
    )
