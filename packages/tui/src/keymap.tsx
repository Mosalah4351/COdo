...
type COdoModeStack = ReturnType<typeof createCOdoModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, COdoModeStack>()
...
export function useCOdoModeStack() {
  return getCOdoModeStack(useCOdoKeymap())
}

export function getCOdoModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("COdo mode stack is not registered for this keymap")
  return value
}
...
export function registerCOdoKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createCOdoModeStack(keymap)
