import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@codo-ai/plugin/tui"
...
import type { useCOdoKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useCOdoKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getCOdoModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getCOdoModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
